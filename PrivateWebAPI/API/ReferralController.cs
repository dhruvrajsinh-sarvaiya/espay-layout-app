    ...
    public class ReferralController : BaseController
    {
        ...
        public IActionResult AddReferralUserClick(AddReferralClickRequest Request)
        {
            try
            {
                ...
                {
                    return Ok(new ReferralUserClickResponse { ReturnCode = enResponseCode.Success, ReturnMsg = EnResponseMessage.CommRecordInsertSuccess, ReferralCode = response });
                }
                else
                {
        ...
        [Authorize]
        public async Task<ActionResult> ListAdminReferralRewards(int PageIndex = 0, int Page_Size = 0, long ReferralServiceId = 0, string UserName = null, DateTime? FromDate = null, DateTime? ToDate = null)
        {
            try
            {
                if (FromDate != null && FromDate > DateTime.UtcNow.Date)
                ...
                var getReferralRewardsData = await Task.FromResult(_ReferralRewards.ListAdminReferralRewards(PageIndex, Page_Size, ReferralServiceId, UserName, FromDate, ToDate));
        ...
        [Authorize]
        public async Task<ActionResult> ListUserReferralRewards(int PageIndex = 0, int Page_Size = 0, long ReferralServiceId = 0, DateTime? FromDate = null, DateTime? ToDate = null)
        {
            try
            {
                if (FromDate != null && FromDate > DateTime.UtcNow.Date)
                ...
                ApplicationUser user = await _userManager.GetUserAsync(HttpContext.User);
                var getReferralRewardsData = await Task.FromResult(_ReferralRewards.ListUserReferralRewards(user.Id, PageIndex, Page_Size, ReferralServiceId, FromDate, ToDate));
                if (getReferralRewardsData != null)
