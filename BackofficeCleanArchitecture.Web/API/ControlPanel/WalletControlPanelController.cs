...
namespace BackofficeCleanArchitecture.Web.API.ControlPanel
{
    ...
    public class WalletControlPanelController : Controller
    {
        ...
        CleanArchitecture.Infrastructure.Data.MultiChainClient _client = null;
        ...
        public async Task<ActionResult> ListMultichainAddress(string chainName, int PageIndex, int PageSize)
        {
            try
            {
                ...
                if (connection != null)
                {
                    _client = new CleanArchitecture.Infrastructure.Data.MultiChainClient(connection.hostname, Convert.ToInt32(connection.port), connection.username, connection.password, connection.chainName);
                    if (_client != null)
                    {
        ...
        [HttpPost]
        public async Task<IActionResult> WithdrawTestring()
        {
            BizResponseClass Response = new BizResponseClass();
            try
            {
                ...
                {
                    Response = await _controlPanelServices.WithdrawTestring();
                }
                return Ok(Response);
            }
            catch (Exception ex)
