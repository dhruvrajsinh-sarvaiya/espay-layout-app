...
module.exports = function validateRegionFromInput(data) {
  ...
  if ((data.regionname) === undefined || (data.regionname) == "" || data.regionname.length == 0 || validator.isEmpty(data.regionname.trim() + '')) {
    errors.regionname = 'region.regionform.error.name';
  } else if ((data.regionname) !== undefined && (data.regionname) !== "" && data.regionname.length > 30) { // Added By Megha Kariya (15/02/2019)
