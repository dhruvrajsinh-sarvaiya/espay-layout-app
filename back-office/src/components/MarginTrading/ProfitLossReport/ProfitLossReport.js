...
class ProfitLossReport extends Component {
    ...
    render() {
        const { drawerClose, ProfitLossReports, intl } = this.props;
        let start_row = ProfitLossReports.length > 0 ? 1 : 0;
                                ...
                                <Table hover className="mb-0" responsive>
                                    ...
                                    <tbody>
                                        {ProfitLossReports.length > 0 ? (
                                            ProfitLossReports.map((item, key) => (
                                                <ProfitLossCollapse
                                                    key={key}
                                                    data={item}
...
const mapStateToProps = ({ ProfitLoassReducer, tradeRecon, actvHstrRdcer, walletUsagePolicy, authTokenRdcer }) => {
    const { loading, ProfitLossReports, TotalCount } = ProfitLoassReducer;
    const { getUser } = actvHstrRdcer;
    const { pairList } = tradeRecon;
    const { walletType } = walletUsagePolicy;
    const { menuLoading, menu_rights } = authTokenRdcer;
    return { loading, ProfitLossReports, pairList, getUser, walletType, TotalCount, menuLoading, menu_rights };
};
