...
class SocialCompanies extends Component {
  ...
  getSocialCompanies() {
    api.get('socialCompanies.js')
      .then((response) => {
        this.setState({ socialCompanie: response.data });
      })
      .catch(error => {
        // error handling
      })
  ...
  render() {
    const { socialCompanie } = this.state;
    return (
      <Scrollbars className="jbs-scroll" autoHeight autoHeightMin={100} autoHeightMax={400} autoHide>
        <div className="table-responsive">
          <table className="table table-hover table-middle">
            ...
            <tbody>
              {socialCompanie && socialCompanie.map((data, key) => (
                <tr key={key}>
                  <td>
