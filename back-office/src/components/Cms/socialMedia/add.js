...
class AddSocialMedia extends Component {
	...
	addSocialMediaDetail() {
		...
		if (!isValid) {
			...
			setTimeout(() => {
				this.props.addNewSocialMedia(data);
	...
	componentWillReceiveProps(nextProps) {
		...
		if (nextProps.cms_SocialMedias_list !== undefined && nextProps.cms_SocialMedias_list !== '') {
			const newMedia = [];
			{
	...
	render() {
		...
		const { err_alert, errors, addSocialMediaDetail, loading, btn_disabled, selectedSocialMedias } = this.state;
					...
					{(menudetail["9EBD9AE5-7103-A522-8F33-96D121504075"] && menudetail["9EBD9AE5-7103-A522-8F33-96D121504075"].Visibility === "E925F86B") && //9EBD9AE5-7103-A522-8F33-96D121504075
						<FormGroup className="mt-20">
							...
							<Input
								...
								{socialMediaArray.map((media, key) => {
									if (selectedSocialMedias.indexOf(media.value) === -1) {
										return (
											<IntlMessages key={key} id={media.text}>{(selectOption) => <option key={key} value={media.value}>{selectOption}</option>}</IntlMessages>
										)
									}
								})}
							</Input>
							{errors && errors.social_media_type && <span className="text-danger"><IntlMessages id={errors.social_media_type} /></span>}
						</FormGroup>}
					...
					{addSocialMediaDetail.social_media_type === "1" &&
						<div>
							<FormGroup>
								...
								/>
								{errors && errors.username && <span className="text-danger"><IntlMessages id={errors.username} /></span>}
							</FormGroup>
							...
							<FormGroup>
								...
								/>
								{errors && errors.source && <span className="text-danger"><IntlMessages id={errors.source} /></span>}
							</FormGroup>
						</div>
					}
					{addSocialMediaDetail.social_media_type === "2" &&
						<div>
							<FormGroup>
								...
								/>
								{errors && errors.appId && <span className="text-danger"><IntlMessages id={errors.appId} /></span>}
							</FormGroup>
							...
							<FormGroup>
								...
								/>
								{errors && errors.pageUrl && <span className="text-danger"><IntlMessages id={errors.pageUrl} /></span>}
							</FormGroup>
						</div>
					}
