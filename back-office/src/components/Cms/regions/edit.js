...
class EditRegion extends Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (typeof nextProps.localebit != 'undefined' && nextProps.localebit != '' && nextProps.localebit == 1) {
			...
			var newObj = Object.assign({}, this.state.regiondetail);
			newObj['locale'] = locale;
		   this.setState({regiondetail:newObj})
		...
		if (typeof nextProps.regiondata != 'undefined' && typeof nextProps.regiondata.locale != 'undefined' && nextProps.regiondata != '') {
			...
			var newObject = Object.assign({}, this.state.regiondetail);
			newObj['locale'] = newlocale;
			newObj['_id '] = nextProps.regiondata._id;
			newObj['regionname'] = nextProps.regiondata.regionname;
			newObj['status'] = nextProps.regiondata.status + "";// Change by Khushbu Badheka D:29/01/2019
		   this.setState({regiondetail:newObject})
	...
	render() {
		...
		const { err_alert, language, errors, regiondetail, loading, btn_disabled } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
