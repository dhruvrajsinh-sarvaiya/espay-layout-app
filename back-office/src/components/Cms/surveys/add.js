...
class AddSurvey extends Component {
      ...
      const locale = {};
      {
        nextProps.language && nextProps.language.map((lang, key) => {
          ...
          locale[lang.code] = {
			...
			let newObj = Object.assign({}, this.state.addNewSurveyDetail);
			newObj['locale'] = locale;
			this.setState({ addNewSurveyDetail: newObj });
				}
  ...
  render() {
    ...
    const { err_alert, language, errors, addNewSurveyDetail, loading, btn_disabled } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
    return (
