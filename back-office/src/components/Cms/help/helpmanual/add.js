...
class AddHelpManual extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (typeof nextProps.localebit != 'undefined' && nextProps.localebit != '' && nextProps.localebit == 1) {
      ...
      var newObj = Object.assign({}, this.state.addNewHelpManualDetail);
			newObj['locale'] = locale;
		   this.setState({addNewHelpManualDetail:newObj})
  ...
  render() {
    ...
    const { err_alert, language, errors, addNewHelpManualDetail, loading, modulelist, btn_disabled } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
                  ...
                  {(menudetail["583355A1-0B7B-9685-1C15-C69C985522F6"] && menudetail["583355A1-0B7B-9685-1C15-C69C985522F6"].Visibility === "E925F86B") && //583355A1-0B7B-9685-1C15-C69C985522F6
                    <FormGroup>
                      ...
                      <Input
                        ...
                        {modulelist.length ? modulelist.map((item, mdlKey) => <option key={mdlKey} value={item._id}>{item.locale.en.module_name}</option>) : ''}
                      </Input>
