...
class EditHelpModule extends Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (typeof nextProps.localebit != 'undefined' && nextProps.localebit != '' && nextProps.localebit == 1) {
			...
			let newObj = Object.assign({}, this.state.helpmoduledetail);
			newObj['locale'] = locale;
			this.setState({ helpmoduledetail: newObj });
				}
		...
		if (typeof nextProps.helpmoduledata != 'undefined' && typeof nextProps.helpmoduledata.locale != 'undefined' && nextProps.helpmoduledata != '') {
			...
			let newObject = Object.assign({}, this.state.helpmoduledetail);
			newObject['locale'] = newlocale;
			newObject['_id'] = nextProps.helpmoduledata._id;
			newObject['category_id'] =nextProps.helpmoduledata.category_id;
			newObject['sort_order'] = nextProps.helpmoduledata.sort_order;
			newObject['status'] = nextProps.helpmoduledata.status + '';
			this.setState({ helpmoduledetail: newObject });
	...
	render() {
		...
		const { err_alert, language, errors, helpmoduledetail, loading, btn_disabled } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
