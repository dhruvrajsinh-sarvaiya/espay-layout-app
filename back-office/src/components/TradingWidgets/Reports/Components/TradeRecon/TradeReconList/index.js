                    ...
                    return [
                      ...
                      <div>
                        <Input type="select" name="actionType" id="Select-2" onChange={(event) => this.handleChangeActionType(event, item.TrnNo)}>
                          <IntlMessages id="traderecon.list.selectall">
                            {(select) =>
                              <option select={this.state.selectedTrnNo == 0} value="">{select}</option>
                            }
                          </IntlMessages>
                          {types.map((currency, key) =>
                            (currency.value == 12 && item.IsProcessing == 0 ||
                              ...
                              null
                              :
                              <IntlMessages id={currency.title} key={key}>
                                {(successAndDebit) =>
                                  <option select={(this.state.selectedTrnNo == item.TrnNo) && (currency.value == this.state.actionType)} value={currency.value}>{successAndDebit}</option>
                                }
                              </IntlMessages>
                          )}
                        </Input>
...
const mapStateToProps = ({ actvHstrRdcer, tradeRecon, drawerclose, authTokenRdcer }) => {
  const {
    tradeReconList,
    error,
    loading,
