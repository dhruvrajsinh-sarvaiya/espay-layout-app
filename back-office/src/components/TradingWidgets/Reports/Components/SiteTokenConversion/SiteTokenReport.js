...
const mapStateToProps = ({ actvHstrRdcer, tradingledgerReducer, siteTokenConversion, drawerclose, authTokenRdcer }) => {
    if (drawerclose.bit === 1) {
        setTimeout(function () {
            drawerclose.bit = 2
        }, 1000);
    }
    const { currencyList, baseCurrencyList } = tradingledgerReducer;
