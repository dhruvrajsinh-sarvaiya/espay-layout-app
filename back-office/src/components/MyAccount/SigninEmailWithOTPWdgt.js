...
class SigninEmailWithOTPWdgt extends Component {
    ...
    onSubmit(event) {
        ...
        if (isValid) {
            ...
            getIPAddress().then(function (ipAddress) {
                ...
                self.props.signInWithEmail(reqObj);
            });
        }
    }
    ...
    verifyOTP(event) {
        ...
        if (isValid) {
            ...
            getIPAddress().then(function (ipAddress) {
                reqObj.ipAddress = ipAddress;
                self.props.signInEmailVerifyOTP(reqObj);
            });
        }
    }

    render() {
        const { deviceModel, deviceMsg, loading, err_alert, err_msg, success_alert, success_msg, email, otp_screen, otp, errors } = this.state;
        return (
            <Fragment>
...
// map state to props
const mapStateToProps = ({ signInWithEmai, authTokenRdcer }) => {
    var response = {
        data: signInWithEmai.data,
        loading: signInWithEmai.loading,
        generate_token: signInWithEmai.generate_token,
