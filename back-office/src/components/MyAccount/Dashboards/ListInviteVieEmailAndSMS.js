...
class ListInviteVieEmailAndSMS extends Component {
  state = {
    Data: [],
    open: false,
    getFilter: {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (Flage_Comp) {
      ...
      this.setState({ getFilter: newObj, showReset: false });
      this.props.getReferralInviteByChannel(newObj);
      if (nextProps.pagedata === 1) {
        this.setState({ title: <IntlMessages id="my_account.emialInvite" /> })
      } else if (nextProps.pagedata === 2) {
        this.setState({ title: <IntlMessages id="my_account.smsInvite" /> })
      }
    }
  ...
  render() {
    const { Data, totalCount, title, payTypeList, serviceData, errors } = this.state;
...
const mapStateToProps = ({ ReferralInvitationsReducer, ReferralRewardConfig, drawerclose, authTokenRdcer }) => {
  ...
  return { listReferralInviteByChannelData, listServiceData, payTypeData, loading, drawerclose, menuLoading, menu_rights };
}
