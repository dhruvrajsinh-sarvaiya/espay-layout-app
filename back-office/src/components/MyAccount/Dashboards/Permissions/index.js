...
class SetPermission extends Component {
    ...
    GetChildData = (event, selectedObject) => {
        ...
        this.setState((prevState) => {
            return {
                ...
                showData: true,
                getData: true,
                selectedObject: prevState.selectedObject,
                DrawerData: info
            };
        });
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.menuList.hasOwnProperty('Modules') && Object.keys(nextProps.menuList.Modules).length > 0 && this.state.isExpandSubModule) {
            let checkCount = this.checkAllSubModule(nextProps.menuList.Modules);
        ...
        if (nextProps.menuList.hasOwnProperty('Modules') && Object.keys(nextProps.menuList.Modules).length > 0 && this.state.getListBit === 0) {
            let mainCheckAllCount = this.checkAllSubModule(nextProps.menuList.Modules);
            this.setState({ mainCheckAll: mainCheckAllCount });
        }
    }
    ...
    updateStateData = (type, value, mIdx = '', smIdx = '') => {
        ...
        if (type === 'AllMainMenu') {
            ...
            var checkAllCount = this.checkAllSubModule(newObj.Modules[mIdx]['childs']['Modules']);
            this.setState({ checkAll: checkAllCount });
        }
    ...
    updateModuleAccessRight = (event, type, index) => {
        ...
        if (type === 'MainMenu') {
            this.state.cnfgMdlData.Modules.map((item) => {
                let mData = {
            ...
            this.state.cnfgMdlData.Modules[index]['childs']['Modules'].map((item) => {
                let mData = {
                    ModuleGroupAccessID: item.ModuleGroupAccessID,
                    ModuelID: item.ModuelID,
                    Status: item.Status, //1,
            ...
            <Fragment>
                ...
                <div className="jbs-page-content">
                    ...
                    {menuPermissionDetail.CrudOption.indexOf('0BB7ACAC') !== -1 && // check edit curd operation
                        (<Form>
                            {cnfgMdlData.Modules !== undefined && cnfgMdlData.Modules.length > 0 &&
                                <Fragment>
                                    {/* Added by salim dt:07/05/2019 */}
                                    <div>
                                        <Checkbox
...
const mapStateToProps = ({ GetMenuAccessReducer, drawerclose, authTokenRdcer }) => {
    if (drawerclose.bit === 1) {
        setTimeout(function () { drawerclose.bit = 2 }, 1000);
    }
    const { menuLoading, menu_rights } = authTokenRdcer;
    const { loading, menuList, getListBit, menuListError, updData } = GetMenuAccessReducer;
    return { loading, menuList, drawerclose, getListBit, menuListError, updData, menuLoading, menu_rights };
};
