...
import { affiliateSmsSentReport } from 'Actions/MyAccount';
...
class SendSMSReport extends Component {
    ...
    componentWillMount() {
        this.getSendSMSList(this.state.data.PageNo, this.state.data.PageSize);
    }
    // to apply filter
    clearFilter = () => {
        var newObj = Object.assign({}, this.state.data);
    ...
    handlePageChange = (pageNumber) => {
        this.getSendSMSList(pageNumber);
    }

    onChangeRowsPerPage = event => {
        this.getSendSMSList(1, event.target.value);
    };
        ...
        const options = {
            ...
            onTableChange: (action, tableState) => {
                if (action === 'changeRowsPerPage' || action === 'changePage') {
                    this.getSendSMSList(tableState.page, tableState.rowsPerPage);
                }
            },
            downloadOptions: {
                filename: 'Send_SMS_Report_' + changeDateFormat(new Date(), 'YYYY-MM-DD') + '.csv'
            }
        };
        return (
            <Fragment>
...
const mapStateToProps = ({ affiliateReportRdcer }) => {
    const { smslist, loading } = affiliateReportRdcer;
    return { smslist, loading };
}
