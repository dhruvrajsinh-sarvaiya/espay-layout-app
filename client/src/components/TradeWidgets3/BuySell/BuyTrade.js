...
class BuyOrderRow extends Component {
    ...
    bulkOrderProcess(key) {
        if (key < 999) this.props.openModal(key);
    }
...
class BuyTrade extends Component {
    constructor(props) {
        super(props);
        this.state = {
            buyerOrderList: this.props.buyerOrderList.length
                ? this.props.buyerOrderList
                : [],
            modal: false,
            modalInfo: 0,
            sectionReload: false,
    ...
    render() {
        ...
        if (
            ...
            diffLimit <= buySellRecordCount
        ) {
            for (
                var lastIndex = this.props.buyerOrderList.length;
                lastIndex < buySellRecordCount;
                lastIndex++
            ) {
                buyOrderList.push(
                    <BuyOrderRow
