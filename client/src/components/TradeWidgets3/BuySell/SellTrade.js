...
class SellOrderRow extends Component {
    ...
    bulkOrderProcess(key) {
        if (key < 999) this.props.placeOrder(key);
    }
    ...
    render() {
        ...
        const highDepth =
            this.props.Price !== "-"
                ? parseFloat((this.props.Amount * 100) / sellorDepth).toFixed(2)
                : 0;
        ...
        return (
            <tr
                ...
                style={{
                    cursor: "pointer",
                    background:
                        "linear-gradient(rgba(250,82,82,.15),rgba(250,82,82,.15)) 0% 100%",
...
class SellTrade extends Component {
    constructor(props) {
        super(props);
        this.state = {
            sellerOrderList: this.props.sellerOrderList.length
                ? this.props.sellerOrderList
                : [],
            sectionReload: false,
            showLoader: true,
            oldState: [],
