...
class StopLimitOrder extends React.Component {
    state = {
        ...
        stopBuy: "",
        limitBuy: this.props.currentPrice.LastPrice
            ? parseFloat(this.props.currentPrice.LastPrice).toFixed(8)
            : parseFloat(0).toFixed(8),
        amountBuy: "",
        totalBuy: "",

        stopSell: "",
        limitSell: this.props.currentPrice.LastPrice
            ? parseFloat(this.props.currentPrice.LastPrice).toFixed(8)
            : parseFloat(0).toFixed(8),
        amountSell: "",
        totalSell: "",
    ...
    componentWillReceiveProps(nextprops) {
        if (
            nextprops.currentPrice &&
            nextprops.currentPrice.LastPrice &&
            this.state.lastPriceBit !== nextprops.lastPriceBit
        ) {
            this.setState({
                limitBuy: parseFloat(nextprops.currentPrice.LastPrice).toFixed(
                    8
                ),
                limitSell: parseFloat(nextprops.currentPrice.LastPrice).toFixed(
                    8
                ),
        ...
        if (
            nextprops.bulkBuyOrder &&
            nextprops.bulkBuyOrder.Price &&
            nextprops.bulkBuyOrder.Amount &&
            nextprops.bulkBuyOrder.Total
        ) {
            this.setState({
        ...
        if (
            nextprops.bulkSellOrder &&
            nextprops.bulkSellOrder.Price &&
            nextprops.bulkSellOrder.Amount &&
            nextprops.bulkSellOrder.Total
        ) {
            this.setState({
                priceSell: parseFloat(nextprops.bulkSellOrder.Price).toFixed(8),
                amountSell: parseFloat(nextprops.bulkSellOrder.Amount).toFixed(
                    8
                ),
        ...
        if (nextprops.buyOrder && nextprops.error.length === 0) {
            if (this.state.buyStopOrderBit) {
                if (
                    ...
                    nextprops.buyOrder.ErrorCode === 4566
                ) {
                    NotificationManager.success(
                        <IntlMessages
                            id={`trading.orders.orders.trnid`}
                            values={nextprops.buyOrder.response}
                        />
                    );
                } else if (
                    nextprops.buyOrder.statusCode === 200 &&
                    nextprops.buyOrder.ErrorCode === 4568
                ) {
                    NotificationManager.error(
                        <IntlMessages id="error.trading.transaction.4568" />
                    );
                }
            ...
            if (nextprops.error.ReturnCode === 9) {
                NotificationManager.error(
                    <IntlMessages id="placeorder.buysell.error.internalerror" />
                );
            }
        ...
        if (nextprops.sellOrder && nextprops.error.length === 0) {
            if (this.state.sellStopOrderBit) {
                if (
                    nextprops.sellOrder.statusCode === 200 &&
                    nextprops.sellOrder.ErrorCode === 4566
                ) {
                    NotificationManager.success(
                        <IntlMessages
                            id={`trading.orders.orders.trnid`}
                            values={nextprops.sellOrder.response}
                        />
                    );
                } else if (
                    nextprops.sellOrder.statusCode === 200 &&
                    nextprops.sellOrder.ErrorCode === 4568
                ) {
                    NotificationManager.error(
                        <IntlMessages id="error.trading.transaction.4568" />
                    );
        ...
        if (nextprops.currentBuyPrice && nextprops.currentSellPrice) {
            if (
                ...
                this.setState({
                    ...
                    limitSell: parseFloat(nextprops.currentSellPrice).toFixed(
                        8
                    ),
                });
            }
            ...
            if (isValid) {
                ...
                if (this.state.limitBuy !== "" && this.state.isLimitBuyValid) {
                    this.setState({
                        totalBuy: parseFloat(
                            parseFloat(event.target.value) *
                            parseFloat(this.state.limitBuy)
                        ).toFixed(8),
                        isTotalBuyValid: true,
                    });
                } else {
            ...
            if (isValid) {
                ...
                if (
                    ...
                    this.setState({
                        totalSell: parseFloat(
                            parseFloat(event.target.value) *
                            parseFloat(this.state.limitSell)
                        ).toFixed(8),
                        isTotalSellValid: true,
                    });
                } else {
    ...
    doSellOrder = (event) => {
        ...
        if (
            this.state.stopSell === "" ||
            typeof this.state.stopSell === 'undefined' ||
            this.state.stopSell === 0
        ) {
            this.setState({ showLoader: false, sellStopOrderBit: 0 });
            NotificationManager.error(
                <IntlMessages id="trading.stoplimit.entersellstop" />
            );
        } else if (
            this.state.limitSell === "" ||
            typeof this.state.limitSell === 'undefined' ||
            this.state.limitSell === 0
        ) {
            NotificationManager.error(
                <IntlMessages id="trading.stoplimit.enterselllimit" />
            );
        } else if (
            this.state.amountSell === "" ||
            ...
            this.setState({ showLoader: false, sellStopOrderBit: 0 });
            NotificationManager.error(
                <IntlMessages id="error.trading.transaction.4608" />
            );
        } else if (
            this.state.totalSell === "" ||
            typeof this.state.totalSell === 'undefined' ||
            this.state.totalSell === 0
        ) {
            this.setState({ showLoader: false, sellStopOrderBit: 0 });
            NotificationManager.error(
                <IntlMessages id="error.trading.transaction.4609" />
            );
        } else if (
            info.currencyPairID === "" ||
            ...
            this.setState({ showLoader: false, sellStopOrderBit: 0 });
            NotificationManager.error(
                <IntlMessages id="error.trading.transaction.4601" />
            );
        } else if (
            ...
            this.setState({ showLoader: false, sellStopOrderBit: 0 });
            NotificationManager.error(
                <IntlMessages id="error.trading.creditwallet" />
            );
        } else if (
            ...
            this.setState({ showLoader: false, sellStopOrderBit: 0 });
            NotificationManager.error(
                <IntlMessages id="error.trading.debitwallet" />
            );
        } else {
            const data = {
                ...
                Pair:
                    this.props.firstCurrency + "_" + this.props.secondCurrency,
                feePer: 0,
    ...
    doBuyOrder = (event) => {
        ...
        if (
            this.state.stopBuy === "" ||
            typeof this.state.stopBuy === 'undefined' ||
            this.state.stopBuy === 0
        ) {
            this.setState({ showLoader: false, buyStopOrderBit: 0 });
            NotificationManager.error(
                <IntlMessages id="trading.stoplimit.enterbuystop" />
            );
        } else if (
            this.state.limitBuy === "" ||
            typeof this.state.limitBuy === 'undefined' ||
            this.state.limitBuy === 0
        ) {
            NotificationManager.error(
                <IntlMessages id="trading.stoplimit.enterbuylimit" />
            );
        } else if (
            this.state.amountBuy === "" ||
            ...
            if (!isValid) {
                ...
                if (
                    ...
                    if (
                        this.state.totalBuy <= this.props.secondCurrencyBalance
                    ) {
