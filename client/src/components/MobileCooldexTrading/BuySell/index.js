...
class BuySellTrade extends React.Component {
  ...
  updateStateForDynamicData = () => {
    ...
    if (lastPrice > 0 && lastPriceSellRecordDetail && lastPriceSellRecordDetail.Amount > 0) {
      var findLastPriceIndexVar = sellOrderDetail.findIndex(sellerOrders => parseFloat(sellerOrders.Price) === parseFloat(lastPrice));
    ...
    if (LPSellerBookList && LPSellerBookList.length > 0) {
      LPSellerBookList.forEach(function (LPSellerBooks, index) {
        LPSellerBooks.map((LPSellOrder, indexValue) => {
          var findLPIndex = sellOrderDetail.findIndex(SellerOrders => parseFloat(SellerOrders.Price) === parseFloat(LPSellOrder.Price));
          if (findLPIndex === -1) {
  ...
  render() {    
    ...
    return (
      <div className="mobilebuyselltreade">
        <Row>
          <Col md={8} xs={7}>
