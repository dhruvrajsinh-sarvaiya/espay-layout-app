...
class Announcement extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.annoucements !== undefined) {
      this.setState({ annoucements: nextProps.annoucements });
    }
  }
              ...
              return (
                <ExpansionPanel key={key} className="mb-15 panel">
                  <ExpansionPanelSummary expandIcon={<i className="zmdi zmdi-chevron-down"></i>} className="m-0 panel-heading demo">
                    <Typography className="text-primary">{headline.locale[localStorage.getItem('locale')] && headline.locale[localStorage.getItem('locale')].title}</Typography>
                  </ExpansionPanelSummary>
                  <ExpansionPanelDetails>
                    <div className="d-block">
                      <p>{dateFormat(headline.date_modified, "isoDate")}</p>
                      {ReactHtmlParser(headline.locale[localStorage.getItem('locale')] && headline.locale[localStorage.getItem('locale')].content)}
                    </div>
                  </ExpansionPanelDetails>
                </ExpansionPanel>
              );
...
export default connect(mapStateToProps, { getAnnoucements })(Announcement);
