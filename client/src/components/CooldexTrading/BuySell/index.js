...
// import Action
import { getSellerOrderList, getBuyerOrderList } from 'Actions/Trade';
...
class BuySellTrade extends React.Component {
  ...
  updateStateForDynamicData = () => {
    ...
    if (lastPrice > 0 && lastPriceBuyRecordDetail && lastPriceBuyRecordDetail.Amount > 0) {
      var findLastPriceIndex = buyOrderDetail.findIndex(buyerOrders => parseFloat(buyerOrders.Price) === parseFloat(lastPrice));
    ...
    if (LPBuyerBookList && LPBuyerBookList.length > 0) {
      LPBuyerBookList.forEach(function (LPBuyerBook, index) {
        LPBuyerBook.map((LPBuyOrder, indexValue) => {
          var findLPIndex = buyOrderDetail.findIndex(sellerOrders => parseFloat(sellerOrders.Price) === parseFloat(LPBuyOrder.Price));
          if (findLPIndex === -1) {
    ...
    if (stopLimitSellerBookList && stopLimitSellerBookList.length > 0) {
      stopLimitSellerBookList.map((stopLimitOrder, indexValue) => {
        if (stopLimitOrder.IsAdd && stopLimitOrder.IsAdd === 1) {
          var findStopLimitIndex = sellOrderDetail.findIndex(SellerOrders => parseFloat(SellerOrders.Price) === parseFloat(stopLimitOrder.Price));
          if (findStopLimitIndex === -1) {
            stopLimitOrder.UpDownBit = 1;
    ...
    if (LPSellerBookList && LPSellerBookList.length > 0) {
      LPSellerBookList.forEach(function (LPSellerBooks, index) {
        LPSellerBooks.map((LPSellOrder, indexValue) => {
          var findLPIndex = sellOrderDetail.findIndex(LPsellerOrder => parseFloat(LPsellerOrder.Price) === parseFloat(LPSellOrder.Price));
          if (findLPIndex === -1) {
  ...
  render() {    
    ...
    if (this.state.lastPrice != 'undefined' && this.state.lastPrice != 0) {
      firstPrice = parseFloat(this.state.lastPrice).toFixed(8);
    }
    return (
...
export default connect(mapStateToProps, { getBuyerOrderList, getSellerOrderList, })(BuySellTrade);
