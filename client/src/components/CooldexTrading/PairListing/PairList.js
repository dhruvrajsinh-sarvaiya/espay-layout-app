...
class PairList extends React.Component {
    ...
    render() {
        ...
        if (this.props.displayFavourite && this.state.pairList.length !== 0) {
            pairsRow = [];
            var oldPairList = this.state.oldPairLists;
            ...
            favouriteList.map((value, indexValue) => {
                ...
                var findIndexValuePrice = oldPairList.findIndex(oldPairItem => oldPairItem.CurrentRate === value.CurrentRate);
                var lastPrice = 0;

                var findIndexValuePer = oldPairList.findIndex(oldPairItem => oldPairItem.ChangePer === value.ChangePer);
                var lastChange = 0;

                var findIndexValueVolume = oldPairList.findIndex(oldPairItem => oldPairItem.Volume === value.Volume);
                var lastVolume = 0;

                if (findIndexValuePrice !== -1) {
                    lastPrice = oldPairList[findIndexValuePrice].CurrentRate;
                }

                if (findIndexValuePer !== -1) {
                    lastChange = oldPairList[findIndexValuePer].ChangePer;
                }

                if (findIndexValueVolume !== -1) {
                    lastVolume = oldPairList[findIndexValueVolume].Volume;
                }
