...
class ImageViewWidget extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            isAvailable: true,
            url: props.url != '' ? ServiceUtilConstant.coinListImageUrl + props.url + '.png' : null,
        };
    }
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (!props.url.includes('.png')) {
            oldImage = ServiceUtilConstant.coinListImageUrl + props.url + '.png';
        }
    ...
    render() {
        ...
        if (this.state.isAvailable && !this.state.url.includes('file://')) {
            return (
                <Image
                    resizeMode={'cover'}
                    source={{ uri: this.state.url + '?random_number=' + new Date().getTime() }}
            ...
            return (
                <Image
                    resizeMode={'cover'}
                    source={!ignoreStrings.includes(this.state.url.replace(ServiceUtilConstant.coinListImageUrl, '').replace('.png', '')) ? R.images.IMG_CURRENCY : null}
                    style={style} />
            );
        }
    }
