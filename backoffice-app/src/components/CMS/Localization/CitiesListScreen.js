...
import { getListCityApi, ClearListCountryData } from '../../../actions/CMS/LocalizationActions';

class CitiesListScreen extends Component {
	...
	constructor(props) {
		...
		this.state = {
			titleScreen: R.strings.cities,
			refreshing: false,
			search: '',
			response: [],
			cityListDataState: null,
			isFirstTime: true,
			selectedPage: 1,
			row: [],
		};
	...
	async componentDidMount() {
		...
		if (await isInternet()) {
			//To get callCitiesListApi 
			this.callCitiesListApi()
		}
	}

	//api call for list and filter reset
	callCitiesListApi = async () => {
		...
		if (await isInternet()) {
			...
			//To getListCityApi list
			this.props.getListCityApi({
				PageIndex: 0,
	...
	static getDerivedStateFromProps(props, state) {
		...
		if (CitiesListScreen.oldProps !== props) {
			CitiesListScreen.oldProps = props;
		} else {
			return null;
		}
		...
		if (isCurrentScreen(props)) {
			...
			const { cityListData } = props.data;

			if (cityListData) {
				try {
					//if local cityListData state is null or its not null and also different then new response then and only then validate response.
					if (state.cityListDataState == null || (state.cityListDataState != null && cityListData !== state.cityListDataState)) {

						//if  response is success then store array list else store empty list
						if (validateResponseNew({
							response: cityListData,
							returnCode: cityListData.responseCode,
							isList: true,
						})) {

							let res = parseArray(cityListData.data);
							...
							return {
								...state, cityListDataState: cityListData,
								response: res, refreshing: false,
								row: addPages(cityListData.totalCount)
							};
						} else {
							return { ...state, cityListDataState: cityListData, response: [], refreshing: false, row: [] };
						}
					}
				} catch (e) {
		...
		return (
			<SafeView style={this.styles().container}>
				...
				<CustomToolbar
					...
					onRightMenuPress={() => this.props.navigation.navigate('AddEditCitiesScreen',
						{ onSuccess: this.callCitiesListApi })}
					onSearchText={(input) => this.onSearch(input)}
				/>

				<View style={{ flex: 1, justifyContent: 'space-between' }}>

					{(this.props.data.cityListFetching && !this.state.refreshing)
						?
						<ListLoader />
						:
							...
							<FlatList
								...
								renderItem={({ item, index }) =>
									<CityListItem
										index={index}
										item={item}
										onAction={() => this.props.navigation.navigate('AddEditCitiesScreen',
											{ item, onSuccess: this.callCitiesListApi })}
...
function mapDispatchToProps(dispatch) {
	return {
		//Perform getListCityApi List Action 
		getListCityApi: (request) => dispatch(getListCityApi(request)),
		//Perform ClearListCountryData Action 
		ClearListCountryData: () => dispatch(ClearListCountryData())
	}
}

export default connect(mapStatToProps, mapDispatchToProps)(CitiesListScreen);