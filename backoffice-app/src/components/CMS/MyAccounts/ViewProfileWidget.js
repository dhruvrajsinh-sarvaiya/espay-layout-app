...
import { changeTheme, getIPAddress, parseArray, convertDate, convertTime } from '../../../controllers/CommonUtils';
...
class ViewProfileWidget extends Component {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (isCurrentScreen(props)) {
            ...
            let { updateData: { data }, LoginHistorydata } = props
                ...
                try {
                    ...
                    if (state.LoginHistorydata == null || (state.LoginHistorydata != null && LoginHistorydata !== state.LoginHistorydata)) {
                        ...
                        if (validateResponseNew({ response: LoginHistorydata, isList: true })) {
                            ...
                            let lastLoginTime = '';
                            if (res.length > 0) {
                                lastLoginTime = convertDate(res[0].Date) + ' ' + convertTime(res[0].Date);
                            }
        ...
        return <TouchableWithoutFeedback onPress={this.onPressWidget}>
            <View style={{ flexDirection: 'row', marginLeft: R.dimens.margin_left_right, marginRight: R.dimens.margin_left_right }}>
                ...
                <View style={{ flex: 1, marginLeft: R.dimens.widget_left_right_margin, justifyContent: 'flex-start', alignItems: 'flex-start' }}>
                    ...
                    <TextViewHML style={{ color: R.colors.textPrimary, fontSize: R.dimens.smallText }}>{this.state.email} </TextViewHML>
