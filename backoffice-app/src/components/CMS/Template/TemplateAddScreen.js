...
class TemplateAddScreen extends Component {
    ...
    constructor(props) {
        ...
        let oldCommServiceType
        if (item) {
            if (item.CommServiceTypeID == 1)
                oldCommServiceType = R.strings.sms
            else if (item.CommServiceTypeID == 2)
                oldCommServiceType = R.strings.Email
        }

        this.state = {

            SpinnerTemplateTypeData: [{ value: R.strings.select + ' ' + R.strings.template_type }, { value: R.strings.Email, type: 2 }, { value: R.strings.sms, type: 1 }],
            selectedTemplateType: item == undefined ? R.strings.select + ' ' + R.strings.template_type : oldCommServiceType,
            selectedTemplateTypeId: item == undefined ? '' : item.CommServiceTypeID,
        ...
        if (isEmpty(this.state.selectedTemplateType) || this.state.selectedTemplateType === R.strings.select + ' ' + R.strings.template_type) {
            ...
            if (this.state.isFromUpdate) {
                let request = {
                    ...
                    CommServiceTypeID: this.state.selectedTemplateTypeId,
                    ...
                    <ScrollView keyboardShouldPersistTaps='always' showsVerticalScrollIndicator={false}>
                        <View style={{ paddingLeft: R.dimens.activity_margin, paddingRight: R.dimens.activity_margin, paddingBottom: R.dimens.padding_top_bottom_margin, paddingTop: R.dimens.padding_top_bottom_margin }}>
                            ...
                            <TitlePicker
                                ...
                                onPickerSelect={(item, object) => this.setState({ selectedTemplateType: item, selectedTemplateTypeId: object.type })} />
