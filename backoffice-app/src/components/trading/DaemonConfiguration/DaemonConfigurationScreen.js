...
class DaemonConfigurationScreen extends Component {
                ...
                try {
                    if (daemonConfig != null) {
                        ...
                        if (validateResponseNew({ response: daemonConfig, isList: true })) {
                            ...
                            var res = daemonConfig.Response;
                            var resArr = [];
                            if (!Array.isArray(res)) {
                                resArr.push(res);
                            }
                            return { ...state, response: (Array.isArray(res)) ? res : resArr, refreshing: false };
                        }
                        else {
    ...
    render() {
        ...
        if (this.state.response != undefined) {
            finalItem = this.state.response.filter((item) => (item.StatusText.toLowerCase().includes(this.state.searchInput.toLowerCase())))
        }
        return (
