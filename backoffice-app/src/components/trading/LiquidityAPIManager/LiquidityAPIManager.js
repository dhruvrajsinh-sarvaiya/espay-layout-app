...
class LiquidityAPIManager extends Component {
    ...
    render() {
        ...
        let finalItems = this.state.data.filter(item => (item.Name.toLowerCase().includes(this.state.search)));
...
class LiquidityAPIManagerItem extends Component {
    ...
    render() {
        ...
        let statusColor = item.Status == 1 ? R.colors.successGreen : R.colors.failRed;
