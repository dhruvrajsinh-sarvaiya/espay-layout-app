	...
	render() {
		...
		if (item.ActionStage) {
			...
			for (let i = 0; i < Actions.length; i++) {
				for (let j = 0; j < actionsTypes.length; j++) {
					if (Actions[i] == actionsTypes[j].code) {
						if (!(actionsTypes[j].code == 12 && item.IsProcessing == 0 || actionsTypes[j].code == 13 && item.IsAPITrade == 0))
							dynamicActions.push(actionsTypes)
					}
				}
			}
		}
