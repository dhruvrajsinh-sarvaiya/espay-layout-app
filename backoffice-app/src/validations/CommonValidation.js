				...
				if (rtCode != 0) {
					...
					if (response.ErrorCode !== undefined && response.ErrorCode == 502) {
						dialog = Object.assign({}, dialog, {
							...
							//If msg is same as slow internet msg than set type to 5 otherwise 1
...
// for checking input has valid guid
export function validateGuId(text) {
	try {
		// \w any word character (A-Z, a-z, 0-9).
...
// for checking input has alphabetic and numeric with space value : special character not allowed
export function validateAlphaNumericWithSpace(text) {
	try {
		// \w any word character (A-Z, a-z, 0-9).
...
export function validatePassword(password) {
	try {
		return matches(password, "^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).*$");
	} catch (error) {
		//logger(error.message)
	}
}
...
export function validPercentage(Percentage) {
	try {
		...
		if (Percentage) {
			...
			return matches(Percentage, /^100(\.0{0,8}?)?$|^\d{0,2}(\.\d{0,8})?$/);
		} return false;
	} catch (error) {
		//logger(error.message)
		return false;
...
// for validating html tag 
export function isHtmlTag(string) {
	try {
		//for enter only without html tag data
		let reg = new RegExp(/(<([^>]+)>)/);
...
// for validating Script tag 
export function isScriptTag(string) {
	try {
		//for enter only without script tag data
		let reg = new RegExp(/<script.*?>([\s\S]*?)<\/script>/);
		return reg.test(string);
	} catch (error) {
		//logger(error.message)
		return false;
