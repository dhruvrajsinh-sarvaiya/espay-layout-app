...
export default class StepIndicator extends PureComponent {
		...
		for (let position = 0; position < stepCount; position++) {
			steps.push(
				<TouchableWithoutFeedback key={position} onPress={() => this.stepPressed(position)}>
					<View style={[styles.stepContainer, direction === 'vertical' ? { flexDirection: 'column', marginTop: -R.dimens.normalizePixels(25) } : { flexDirection: 'row' }]}>
	...
	renderStepLabels = () => {
		const { labels, direction, currentPosition, labelRender } = this.props;
		let labelarr
		if (labelRender != undefined) {
			labelarr = labelRender
		} else {
			labelarr = labels
		}
		var labelViews = labelarr.map((label, index) => {
			...
			const selectedStepMainLabelStyle = index === currentPosition ? { color: this.state.customStyles.currentStepMainLabelColor } : { color: this.state.customStyles.mainLabelColor }
			if (labelRender != undefined) {
				return (
					<TouchableWithoutFeedback key={index} onPress={() => this.stepPressed(index)}>
						<View style={[{ flex: 1, width: '100%', padding: R.dimens.widgetMargin, paddingLeft: R.dimens.normalizePixels(10), marginTop: R.dimens.normalizePixels(8) },]}>
							<View style={{ flexDirection: 'row', alignItems: 'center', }}>
								<Text style={[styles.stepLabel, selectedStepMainLabelStyle, { fontSize: this.state.customStyles.labelSize }, { fontSize: this.state.customStyles.headerTextSize, fontWeight: this.state.customStyles.headerFontWeight }]}>{label.level}</Text>
							</View>
							{label.description && <Text style={[selectedStepLabelStyle, { fontSize: this.state.customStyles.labelSize - 3 }]}>{label.description}</Text>}
							{label.invested && <Text style={[selectedStepLabelStyle, { fontSize: this.state.customStyles.labelSize - 3 }, { fontSize: this.state.customStyles.contentTextSize, color: this.state.customStyles.contentTextColor, }]}>{label.stringInvested}{label.invested}</Text>}
							<Text style={[selectedStepLabelStyle, { fontSize: this.state.customStyles.labelSize - 3 }, { fontSize: this.state.customStyles.contentTextSize, color: this.state.customStyles.contentTextColor, }]}>{label.stringProfit}<Text style={{ color: label.profitBit == 1 ? this.state.customStyles.successGreen : this.state.customStyles.failRed }}>{label.profit}</Text></Text>
						</View>
					</TouchableWithoutFeedback>
				)
			} else {
		...
		switch (this.getStepStatus(position)) {
			case STEP_STATUS.CURRENT: {
				stepStyle = {
					...
					marginTop: R.dimens.normalizePixels(10)
				};
