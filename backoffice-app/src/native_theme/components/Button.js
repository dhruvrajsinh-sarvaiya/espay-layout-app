...
import { mergeStyle, cloneObject } from '../../controllers/CommonUtils';
...
class Button extends Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (this.props.isGradient === nextProps.isGradient &&
            this.props.isRound === nextProps.isRound &&
...
class ButtonOriginal extends Component {
    render() {
        ...
        let rippleStyle = cloneObject(buttonStyle);

        // if isGradient property is not passed or isGradient property is false then show normal button
        if (typeof this.props.isGradient === 'undefined' || this.props.isGradient == false) {
            return <Ripple
                rippleOpacity={1}
            ...
            // Show Gradient button because both of above condition are false and isGradient is true in props
            // Changes needed for gradients are below
            // Removing unnecessary style from ripple style so that gradient fits properly
            delete rippleStyle['elevation'];
            delete rippleStyle['backgroundColor'];
            delete rippleStyle['marginTop'];
            delete rippleStyle['marginBottom'];

            return <LinearGradient
                style={[buttonStyle, { paddingLeft: 0, paddingRight: 0 }]}
                locations={[0, 1]}
