...
import { logger, showSlowInternetDialog, showTimeoutRequestDialog, showTryAfterSometimeRequestDialog, getBaseUrl, getWebPageUrl } from '../controllers/CommonUtils';
...
/**
 * Function to Swagger Put API
 */
export const swaggerPutAPI = async (methodName, request, headers = {}) => {

    const httpClient = axios.create();
    httpClient.defaults.timeout = requestTimeout;
    httpClient.defaults.headers.common = headers;

    logRequest(ServiceUtilConstant.swaggerURL, 'PUT', methodName, request, headers);

    var responseData = await httpClient.put(ServiceUtilConstant.swaggerURL + methodName, request)
    ...
    var responseData = await httpClient.get(getWebPageUrl() + methodName, request)
        .catch(error => {
            if (error.message.includes(requestTimeoutError)) {
                ...
                // showSlowInternetDialog()
                showTryAfterSometimeRequestDialog();
            }
        });
...
/**
 * Function to WebPageUrl put API
 */
export const WebPageUrlPutAPI = async (methodName, request, headers = {}) => {
    const httpClient = axios.create();
    httpClient.defaults.timeout = requestTimeout;
    httpClient.defaults.headers.common = headers;

    logRequest(ServiceUtilConstant.webPageUrl, 'PUT', methodName, request, headers);

    var responseData = await httpClient.put(ServiceUtilConstant.webPageUrl + methodName, request)
        .then(response => JSON.parse(JSON.stringify(response)))
        .catch(error => {
            if (error.message.includes(requestTimeoutError)) {
                showTimeoutRequestDialog();
