        ...
        faqcategories.save()
          .then(data => {
            ...
            res.status(500).send({
              ...
              errors: { "message": "common.api.internalerror" },
              isError: true,
              message: error.message || "Some error occurred while creating the addFaqCategory."
            });
            errorLog(error, "addFaqCategory");
          });
      }).catch(errs => {
        res.status(500).send({
          responseCode: 9,
          errors: { "message": "common.api.internalerror" },
          isError: true,
          message: errs.message || "Some error occurred while retrieving addFaqCategory."
        });
        errorLog(errs, "addFaqCategory");
      });
    });
  } catch (err) {
  ...
  try {
    ...
    FaqCategories.findOne({ _id: req.params.faqcategoryId })
      .then(faqcategories => {
        ...
        errorLog(error, "GetFaqCategoryById");
      ...
      if (totalquestion > 0) {
        ...
        FaqCategories.findOneAndDelete({ _id: req.params.faqcategoryId })
          .then(faqcategories => {
            ...
            errorLog(errs, "DeleteFaqCategory");
            return res.status(500).send({
              responseCode: 9, isError: true, errors: { "message": "common.api.internalerror" },
