        ...
        faqquestions.save()
          .then(data => {
            ...
            res.status(500).send({
              ...
              errors: {},
              isError: true,
              message: error.message || "Some error occurred while creating the addFaqQuestion."
            });
            errorLog(error, "addFaqQuestion");
          });
      }).catch(error => {
        res.status(500).send({
          responseCode: 9,
          errors: { "message": "common.api.internalerror" },
          isError: true,
          message: error.message || "Some error occurred while retrieving addFaqQuestion."
        });
        errorLog(error, "addFaqQuestion");
      });
    });
...
router.get('/faqCount', function (req, res) {
  ...
  try {
    ...
    FaqQuestions.countDocuments({}).then(Questions => {
      count.FaqQuestions = Questions;
