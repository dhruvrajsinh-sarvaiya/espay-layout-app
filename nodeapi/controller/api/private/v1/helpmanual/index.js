        ...
        helpmanuals.save()
          .then(data => {
            ...
            res.status(500).send({
              ...
              errors: {},
              isError: true,
              message: error.message || "Some error occurred while creating the addHelpManual."
            });
            errorLog(error, "addHelpManual");
          });
      }).catch(error => {
        res.status(500).send({
          responseCode: 9,
          errors: { "message": "common.api.internalerror" },
          isError: true,
          message: error.message || "Some error occurred while retrieving addHelpManual."
        });
        errorLog(error, "addHelpManual");
      });
    });
