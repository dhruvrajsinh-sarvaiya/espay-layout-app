  ...
  try {
    if (req.files !== undefined && req.files != null) {
      if (req.files.logo !== undefined && req.files.logo != '') {
        ...
        let logofilename = logooriginalname.substr(0, logooriginalname.lastIndexOf('.'));
        logo_name = logofilename + Date.now() + '.' + logoextension[logoextension.length - 1];
      ...
      if (typeof req.files.fevicon != 'undefined' && req.files.fevicon != '') {
        ...
        let logofilename = feviconoriginalname.substr(0, feviconoriginalname.lastIndexOf('.'));
        fevicon_name = logofilename + Date.now() + '.' + feviconextension[feviconextension.length - 1];
...
app.get('/getIpAddress', function (req, res) {
  ...
  try {
    ...
    res.status(500).send({ responseCode: 1, message: "IPAddress Not Found" });
  }
});
