                ...
                AdvanceHTMLBlock.save()
                    .then(data => {
                        ...
                        res.status(500).send({
                            ...
                            errors: { "message": "common.api.internalerror" },
                            isError: true,
                            message: error.message || "Some error occurred while creating the HTML Block."
                        });
                        errorLog(error, "AddAdvanceHTMLBlocks");
                    });
            }).catch(errors => {
                res.status(500).send({
                    responseCode: 9,
                    errors: { "message": "common.api.internalerror" },
                    isError: true,
                    message: errors.message || "Some error occurred while retrieving HTML Block."
                });
                errorLog(errors, "AddAdvanceHTMLBlocks");
            });
        });
    } catch (err) {
        errorLog(err, "AddAdvanceHTMLBlocks");
