                ...
                ImageSlider.save()
                    .then(data => {
                        ...
                        res.status(500).send({
                            ...
                            errors: { "message": "common.api.internalerror" },
                            isError: true,
                            message: error.message || "Some error occurred while creating the Image Slider."
                        });
                        errorLog(error, "AddImageSldier");
                    });
            }).catch(error => {
                res.status(500).send({
                    responseCode: 9,
                    error: { "message": "common.api.internalerror" },
                    isError: true,
                    message: error.message || "Some error occurred while retrieving Image Slider."
                });
                errorLog(error, "AddImageSldier");
            });
        });
    ...
    try {
        ...
        ImageSliders.findOneAndUpdate({ sliderid: imageSlider.sliderid }, {
            ...
            .then(imageSliders => {
                ...
                responseLog({ responseCode: 0, isError: false, message: 'Image Slider Edit successfullly.', data: imageSliders }, "EditImageSlider");
                const content = JSON.stringify(output);
                fs.writeFileSync(filepath + setting.siteid + '.json', content);
            }).catch(err => {
