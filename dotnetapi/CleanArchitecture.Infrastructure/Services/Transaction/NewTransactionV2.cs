...
{
    public class NewTransactionV2 : ITransactionProcessV1
    {
        ...
        public NewTransactionV2(
            ...
            ICommonRepository<TransactionRequest> TransactionRequest, IMediator mediator, IBinanceLPService BinanceLPService,
            IBitrexLPService BitrexLPService, ICoinBaseService CoinBaseService, IPoloniexService PoloniexService, 
            ITradeSatoshiLPService TradeSatoshiLPService)
        {
        ...
        public async Task<BizResponse> CombineAllInitTransactionAsync()
        {
            ...
            try
            {
                ...
                //khushali 22-01-2019 move to local trading
                //if (Req.TrnType == enTrnType.Buy_Trade)
                //{
                ...
                //    }
                //}
                TradingDataInsertV2(_Resp);
                Task.Delay(20000).Wait();//rita 3-1-19 wait for all operations done //Uday 15-01-2019 change from 10000 to 20000
                return _Resp;
            }
            catch (Exception ex)
            {
        ...
        public async Task<BizResponse> TradingDataInsertV2(BizResponse _Resp)
        {
            ...
            //ThirdPartyAPIRequest ThirdPartyAPIRequestOnj; //komal 03 May 2019, Cleanup
            //WebApiConfigurationResponse WebApiConfigurationResponseObj; //komal 03 May 2019, Cleanup
            ...
            try
            {
                ...
                foreach (TransactionProviderResponse Provider in TxnProviderList)//Make txn on every API
                {
                    switch (Enum.Parse<enWebAPIRouteType>(Provider.ProTypeID.ToString()))
                    {
                        ...
                        case enWebAPIRouteType.LiquidityProvider: // for liquidity provider
                            ...
                            switch (Provider.AppTypeID)
                            {
                                case (long)enAppType.Binance:
                                    ...
                                    {
                                        if (symbol.Name == ( _BaseCurrService.SMSCode+_SecondCurrService.SMSCode))
                                        {
                                            if (symbol.MinNotionalFilter.MinNotional <= Req.Price * Req.Amount)
                                            {
                                                if (symbol.PriceFilter.MinPrice <= Req.Price && symbol.PriceFilter.MaxPrice >= Req.Price)
                                                {
                                                    if (symbol.MarketLotSizeFilter.MinQuantity <= Req.Amount && symbol.MarketLotSizeFilter.MaxQuantity >= Req.Amount)
                                                    {
                                                        ...
                                                        CallResult<BinancePlacedOrder> BinanceResult = await _binanceLPService.PlaceOrderAsync(Req.TrnType == enTrnType.Sell_Trade ? Binance.Net.Objects.OrderSide.Sell : Binance.Net.Objects.OrderSide.Buy, Provider.OpCode, type, Req.Amount, price: Req.Price, stopPrice: Req.StopPrice);
                                ...
                                case (long)enAppType.Bittrex:
                                    ...
                                    CallResult<BittrexGuid> BittrexResult1 =  await _bitrexLPService.PlaceOrderAsync(Req.TrnType == enTrnType.Sell_Trade ? Bittrex.Net.Objects.OrderSide.Sell : Bittrex.Net.Objects.OrderSide.Buy, Provider.OpCode, Req.Amount, Req.Amount);
                                ...
                                case (long)enAppType.TradeSatoshi:
                                    ...
                                    SubmitOrderReturn TradeSatoshiResult =  await _tradeSatoshiLPService.PlaceOrderAsync(Req.TrnType == enTrnType.Sell_Trade ? Core.Interfaces.LiquidityProvider.OrderSide.Sell : Core.Interfaces.LiquidityProvider.OrderSide.Buy, Provider.OpCode, Req.Amount, Req.Price);
                                ...
                                case (long)enAppType.Poloniex:
                                    ...
                                    var PoloniexRes = await _poloniexService.PlacePoloniexOrder(_BaseCurrService.SMSCode,_SecondCurrService.SMSCode,Req.Amount,Req.Price, Req.TrnType == enTrnType.Sell_Trade ? enOrderType.SellOrder : enOrderType.BuyOrder);
                                ...
                                case (long)enAppType.Coinbase:
                                    ...
                                    OrderResponse CoinbaseResult = await _coinBaseService.PlaceOrder(Req.ordertype, Req.TrnType == enTrnType.Sell_Trade ? CoinbasePro.Services.Orders.Types.OrderSide.Sell : CoinbasePro.Services.Orders.Types.OrderSide.Buy, _SecondCurrService + "_" + _BaseCurrService, Req.Amount,Req.Price,Req.StopPrice);
