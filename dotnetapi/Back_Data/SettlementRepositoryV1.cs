...
{
    public class SettlementRepositoryV1: ISettlementRepositoryV1<BizResponse>
    {
        ...
        string ControllerName = "SettlementRepositoryV1";
        ...
        public SettlementRepositoryV1(CleanArchitectureContext dbContext, ICommonRepository<TradePoolQueue> TradePoolQueue,
        ...
        {           
            ...
            try
            {
                ...
                Task<IEnumerable<TradeSellerListV1>> MatchSellerListBaseAllResult = _TradeSellerList.FindByAsync(e=>e.Status == Convert.ToInt16(enTransactionStatus.Initialize) || e.Status == Convert.ToInt16(enTransactionStatus.Hold));
                ...
                (from u in MatchSellerListBaseAll where u.OrderType == 2 select u).ToList()
                ...
                foreach (TradeSellerListV1 SellerList in MatchSellerListBase)//MatchSellerList
                {
                    ...
                    if (SellerList.RemainQty < CurrentTradeBuyerListObj.RemainQty)//Do not Release Qty here
                    {
                        ...
                        if (await WalletResult == false)
                        {
                            ...
                            var CallspReverseResult = Callsp_TradeSettlement(CurrentTradeBuyerListObj.TrnNo, SellerList.TrnNo, SettlementQty, BaseCurrQty, SettlementPrice, 11, 2);
                    ...
                    else if (SellerList.RemainQty >= CurrentTradeBuyerListObj.RemainQty && CurrentTradeBuyerListObj.RemainQty != 0)
                    {
                        ...
                        decimal TakeDisc = 0;
                        InsertTradePoolQueue(TransactionQueueObj.MemberID, CurrentTradeBuyerListObj.PairID, SellerList.TrnNo, SellerList.RemainQty, SettlementPrice, CurrentTradeBuyerListObj.TrnNo, SettlementQty, SettlementPrice, TakeDisc, 0, "SELL", "BUY");//+ SettlementQty//CurrentTradeBuyerListObj.Price , buy on price of seller's price
                        ...
                        {
                            await MakeTradeStopLossObjResult;//for no contex error
                            var CallspReverseResult = Callsp_TradeSettlement(CurrentTradeBuyerListObj.TrnNo, SellerList.TrnNo, SettlementQty, BaseCurrQty, SettlementPrice, 12, 2);
                            _dbContext.Entry(CurrentTradeBuyerListObj).Reload();
                            _dbContext.Entry(TransactionQueueObj).Reload();
                            _dbContext.Entry(TradeTransactionQueueObj).Reload();
        ...
        public async Task<BizResponse> PROCESSSETLLEMENTSell(BizResponse _Resp, TradeTransactionQueue TradeTransactionQueueObj,TransactionQueue TransactionQueueObj, TradeStopLoss TradeStopLossObj,TradeSellerListV1 CurrentTradeSellerListObj, string accessToken = "", short IsCancel = 0)
        {
            ...
            try
            {
                ...
                Task<IEnumerable<TradeBuyerListV1>> MatchSellerListBaseAllResult = _TradeBuyerList.FindByAsync(e => e.Status == Convert.ToInt16(enTransactionStatus.Initialize) || e.Status == Convert.ToInt16(enTransactionStatus.Hold));
                if (TradeTransactionQueueObj.IsCancelled == 1 || IsCancel == 1)
                {
                    _Resp.ErrorCode = enErrorCode.CancelOrder_OrderalreadyCancelled;
                    _Resp.ReturnCode = enResponseCodeService.Fail;
               ...
               // MatchBuyerListBaseAll.Select(c => { c.Price = pairStastics.LTP; return c; }).ToList();
                (from u in MatchBuyerListBaseAll where u.OrderType == 2 select u).ToList()
                        .ForEach(u => u.Price = pairStastics.LTP);
                ...
                foreach (TradeBuyerListV1 BuyerList in MatchBuyerListBase)//MatchSellerList
                {
                    ...
                    if (BuyerList.RemainQty < CurrentTradeSellerListObj.RemainQty)
                    {
                       ...
                       // var MakeTradeStopLossObjResult = _TradeStopLoss.GetSingleAsync(e => e.TrnNo == TQMakerObj.Id);
                        ...
                        {
                            await MakeTradeStopLossObjResult;//for no contex error
                            var CallspReverseResult = Callsp_TradeSettlement(CurrentTradeSellerListObj.TrnNo, BuyerList.TrnNo, SettlementQty, BaseCurrQty, SettlementPrice, 13, 2);
                    ...
                    else if (BuyerList.RemainQty >= CurrentTradeSellerListObj.RemainQty && CurrentTradeSellerListObj.RemainQty != 0)
                    {
                        ...
                        decimal TakeDisc = 0;
                        InsertTradePoolQueue(TransactionQueueObj.MemberID, CurrentTradeSellerListObj.PairID, BuyerList.TrnNo, BuyerList.RemainQty, SettlementPrice, CurrentTradeSellerListObj.TrnNo, SettlementQty, SettlementPrice, TakeDisc, 0, "BUY", "SELL");//+SettlementQty//BuyerList.Price selled base on seller's price
                        ...
                        {
                            await MakeTradeStopLossObjResult;//for no contex error
                            var CallspReverseResult = Callsp_TradeSettlement(CurrentTradeSellerListObj.TrnNo, BuyerList.TrnNo, SettlementQty, BaseCurrQty, SettlementPrice, 14, 2);
                            _dbContext.Entry(CurrentTradeSellerListObj).Reload();
                            _dbContext.Entry(TransactionQueueObj).Reload();
                            _dbContext.Entry(TradeTransactionQueueObj).Reload();                          
                            //_dbContext.Entry(BuyerList).Reload();
        ...
        public async Task WalletFailOrderProcessBUY(long TrnNo, TradeSellerListV1 SellerList, TradeTransactionQueue TradeTQMakerObj, TransactionQueue TQMakerObj, WalletDrCrResponse CreditWalletResult)
        ...
        private async Task<bool> CheckIsProcessingBUY(long TrnNo,TradeSellerListV1 SellerList,decimal LTP)
        {
            try
            {
                ...
                if (SellerList.IsProcessing == 1 || SellerList.RemainQty <= 0)
                {
                    Task.Run(() => HelperForLog.WriteLogIntoFileAsyncDtTm("CheckIsProcessingBUY Buy skip IsProcessing " + SellerList.IsProcessing + " Remain Qty " + SellerList.RemainQty + " With " + SellerList.TrnNo, ControllerName, "##TrnNo " + TrnNo, Helpers.UTC_To_IST()));
                    return false;
                }
        ...
        private async Task<bool> CheckIsProcessingSELL(long TrnNo, TradeBuyerListV1 BuyerList,decimal LTP)
        {
            try
            {
                ...
                if (BuyerList.IsProcessing == 1 || BuyerList.RemainQty <= 0)
                {
                    Task.Run(() => HelperForLog.WriteLogIntoFileAsyncDtTm("CheckIsProcessingSELL Sell skip IsProcessing " + BuyerList.IsProcessing + " Remain Qty " + BuyerList.RemainQty + " With " + BuyerList.TrnNo, ControllerName, "##TrnNo " + TrnNo, Helpers.UTC_To_IST()));
                    return false;
                }
        ...
        public async Task FullSettleBuySignalR(long TrnNo, TransactionQueue TransactionQueueObj, TradeTransactionQueue TradeTransactionQueueObj,TradeStopLoss TradeStopLossObj, TradeStopLoss MakeTradeStopLossObj, TransactionQueue TQMakerObj, TradeTransactionQueue TradeTQMakerObj,string accessToken, decimal SettlementPrice, decimal SettlementQty,TradeSellerListV1 SellerList, short OrderType)
        {
            try
            {
                ...
                _ISignalRService.OnStatusSuccess(Convert.ToInt16(enTransactionStatus.Success), TransactionQueueObj, TradeTransactionQueueObj, "", TradeStopLossObj.ordertype);//komal
                //Uday 06-12-2018 Send SMS When Transaction is Settled
                //Send SMS For Fully Settled Transaction
                _ISignalRService.OnLtpChange(SettlementPrice, TradeTQMakerObj.PairID, TradeTQMakerObj.PairName);//rita 11-1-19 for Stop and Limit add/remove in book
        ...
        private async Task<bool> WalletProcessBuy(TradeTransactionQueue TradeTQueueCurrentObj, TradeTransactionQueue TradeTQueueMakerObj, decimal Price,decimal SettledQty,decimal BaseCurrQty,short CurrIsFullSettled,short MakerIsFullSettled, short TrnMode, WalletDrCrResponse CreditWalletResult)
        {
            try
            {
        ...
        private async Task<bool> WalletProcessSell(TradeTransactionQueue TradeTQueueCurrentObj, TradeTransactionQueue TradeTQueueMakerObj, decimal Price, decimal SettledQty, decimal BaseCurrQty, short CurrIsFullSettled, short MakerIsFullSettled, short TrnMode,WalletDrCrResponse CreditWalletResult)
        {
            try
            {
        ...
        public async Task<bool> Callsp_TradeSettlement(long TrnNo,long MakerTrnNo,decimal SettlementQty,decimal BaseCurrQty,decimal SettlementPrice,short ActionType,short ActionStage)
        {
            ...
            try
            {
                ...
                new SqlParameter("@SettlementQty",SqlDbType.Decimal, 18, ParameterDirection.Input, false, 18, 8, string.Empty, DataRowVersion.Default, SettlementQty),
                new SqlParameter("@BaseCurrQty",SqlDbType.Decimal, 18, ParameterDirection.Input, false, 18, 8, string.Empty, DataRowVersion.Default, BaseCurrQty),
                new SqlParameter("@SettlementPrice",SqlDbType.Decimal, 18, ParameterDirection.Input, false, 18, 8, string.Empty, DataRowVersion.Default, SettlementPrice),
                ...
                new SqlParameter("@ErrorCode",SqlDbType.BigInt, 10, ParameterDirection.Output, false, 0, 0, string.Empty, DataRowVersion.Default, ErrorCode)
            };
                var res = _dbContext2.Database.ExecuteSqlCommand("Sp_TradeSettlement @TrnNo ,@MakerTrnNo,@SettlementQty,@BaseCurrQty,@SettlementPrice,@ActionType,@ActionStage,@ReturnCode  OUTPUT,@ErrorCode  OUTPUT", param1);
                ...
                Task.Run(() => HelperForLog.WriteLogIntoFileAsync("Sp_TradeSettlement",ControllerName, "##TrnNo:" + TrnNo + " ,MakerTrnNo=" + MakerTrnNo + 
                    ",SettlementQty=" + SettlementQty + ",BaseCurrQty=" + BaseCurrQty + ",SettlementPrice=" + SettlementPrice + ",ActionType=" + ActionType + ",ActionStage=" + ActionStage + ",ReturnCode=" + ReturnCode + ",ErrorCode=" + ErrorCode));
