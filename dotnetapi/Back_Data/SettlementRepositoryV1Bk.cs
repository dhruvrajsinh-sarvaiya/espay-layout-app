...
{
    public class SettlementRepositoryV1Bk //: ISettlementRepositoryV1<BizResponse>
    {
        ...
        string ControllerName = "SettlementRepositoryBkV1";
        ...
        public SettlementRepositoryV1Bk(CleanArchitectureContext dbContext, ICommonRepository<TradePoolQueue> TradePoolQueue,
            ...
            try
            {
                ...
                foreach (TradeSellerListV1 SellerList in MatchSellerListBase)//MatchSellerList
                {
                    ...
                    if (SellerList.OrderType == 2)//Market Order
                    {
                        ...
                        {
                            Task.Run(() => HelperForLog.WriteLogIntoFile("PROCESSSETLLEMENT Buy Maker LTP " + pairStastics.LTP + " > Curr Price " + CurrentTradeBuyerListObj.Price, ControllerName, _Resp.ReturnMsg));
                            continue;
                        }
                    ...
                    if (SellerList.RemainQty < CurrentTradeBuyerListObj.RemainQty)//Do not Release Qty here
                    {
                        ...
                        decimal TakeDisc = 0;
                        InsertTradePoolQueue(TransactionQueueObj.MemberID, CurrentTradeBuyerListObj.PairID, SellerList.TrnNo, SellerList.RemainQty, SettlementPrice, CurrentTradeBuyerListObj.TrnNo, SettlementQty, SettlementPrice, TakeDisc, 0, "SELL", "BUY");//CurrentTradeBuyerListObj.Price , buy on price of seller's price
                        ...
                        var WalletResult = WalletProcessBuy(TradeTransactionQueueObj, TradeTQMakerObj, SettlementPrice, SettlementQty, BaseCurrQty, 0, 1, TransactionQueueObj.TrnMode);
                        ...
                        if (await WalletResult == false)
                        {
                            ...
                            Task.Run(()=> HelperForLog.WriteLogIntoFile("PROCESSSETLLEMENT", ControllerName, _Resp.ReturnMsg));
                        }
                        else
                        {
            ...
            try
            {
                ...
                foreach (TradeBuyerListV1 BuyerList in MatchBuyerListBase)//MatchSellerList
                {
                    ...
                    if (BuyerList.OrderType == 2)//Market Order
                    {
                        ...
                        {
                            Task.Run(() => HelperForLog.WriteLogIntoFile("PROCESSSETLLEMENT Sell skip Maker LTP " + pairStastics.LTP + " < Curr Price " + CurrentTradeSellerListObj.Price, ControllerName, _Resp.ReturnMsg));
                            continue;
                        }
                    ...
                    if (BuyerList.RemainQty < CurrentTradeSellerListObj.RemainQty)
                    {
                        ...
                        //Here Bid Price of Seller always low then user given in Order , base on above Query
                        decimal TakeDisc = 0;
                        InsertTradePoolQueue(TransactionQueueObj.MemberID,CurrentTradeSellerListObj.PairID, BuyerList.TrnNo, BuyerList.RemainQty, SettlementPrice, CurrentTradeSellerListObj.TrnNo, SettlementQty, SettlementPrice, TakeDisc, 0, "BUY", "SELL");//BuyerList.Price selled base on seller's price
                        ...
                        {//last loop records does not revert, and commit in currecnt txn , as it is present in contex
                            ...
                            //HelperForLog.WriteLogIntoFile("PROCESSSETLLEMENT SELL", ControllerName, "Partial Settlement Done with " + BuyerList.TrnNo + "##TrnNo:" + CurrentTradeSellerListObj.TrnNo);
                            _Resp.ErrorCode = enErrorCode.Settlement_PartialSettlementDone;
                            _Resp.ReturnCode = enResponseCodeService.Success;
        ...
        private async Task<Boolean> WalletProcessBuy(TradeTransactionQueue TradeTQueueCurrentObj, TradeTransactionQueue TradeTQueueMakerObj, decimal Price,decimal SettledQty,decimal BaseCurrQty,short CurrIsFullSettled,short MakerIsFullSettled, short TrnMode)
        {
            try
            {
               ...
               var CreditWalletResult = await _WalletService.GetWalletCreditDrForHoldNewAsyncFinal(CrDrSecondCurrObj, CrDrFirstCurrObj,
                ...
                {
                    Task.Run(()=> HelperForLog.WriteLogIntoFile("PROCESSSETLLEMENT WalletProcessBuy", ControllerName, "Wallet Operation fail " + CreditWalletResult.ReturnMsg + " With TrnNo:" + TradeTQueueMakerObj.TrnNo + "##TrnNo:" + TradeTQueueCurrentObj.TrnNo));
                    return false;
                }
        ...
        private async Task<Boolean> WalletProcessSell(TradeTransactionQueue TradeTQueueCurrentObj, TradeTransactionQueue TradeTQueueMakerObj, decimal Price, decimal SettledQty, decimal BaseCurrQty, short CurrIsFullSettled, short MakerIsFullSettled, short TrnMode)
        {
            try
            {
                ...
                //CrFirstCurrObj.trnType = enWalletTrnType.Cr_Trade;
        ...
        //public async Task<BizResponse> CancellationProcessV1(BizResponse _Resp, TransactionQueue TransactionQueueObj, TradeTransactionQueue TradeTransactionQueueObj)
        //{
        ...
        //    try
        //    {
        //        tradeCancelQueue = _TradeCancelQueueRepository.GetSingle(e => e.TrnNo == TradeTransactionQueueObj.TrnNo);
        ...
        public async Task SMSSendSettledTransaction(long TrnNo, string MobileNumber, decimal Price, decimal Qty, int SettleType)
        {
            try
            {
                if (!string.IsNullOrEmpty(MobileNumber))
                {
                    ...
                    Task.Run(() => HelperForLog.WriteLogIntoFile("SendSMSTransaction - SMSSendSettledTransaction", ControllerName, " ##TrnNo : " + TrnNo + " ##MobileNo : " + MobileNumber + " ##Price : " + Price + " ##Qty : " + Qty + " ##Type : " + SettleType));
        ...
        public async Task EmailSendPartialSettledTransaction(long TrnNo,string UserId,string Pair,decimal Qty,string TrnDate,decimal RemainingQty,decimal SettleQty,decimal Amount,decimal Fee)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(User.Email))
                {
                    Task.Run(() => HelperForLog.WriteLogIntoFile("SendEmailTransaction - EmailSendPartialSettledTransaction", ControllerName, " ##TrnNo : " + TrnNo));
