...
class ViewProfileWidget extends Component {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        try {
            let firstNameProp = props.preference[ServiceUtilConstant.FIRSTNAME];
            let lastNameProp = props.preference[ServiceUtilConstant.LASTNAME];
            let userNameProp = props.preference[ServiceUtilConstant.KEY_USER_NAME];
            let emailProp = props.preference[ServiceUtilConstant.Email];
            let avatarProp = props.preference[ServiceUtilConstant.KEY_USER_AVATAR];

            let fullNameProp = '-';
            let userAvatarProp = '';

            //if firstName & lastName is not empty than store name
            if (!isEmpty(firstNameProp) && !isEmpty(lastNameProp)) {
                fullNameProp = firstNameProp + ' ' + lastNameProp;
            }

            if (isEmpty(emailProp) && isEmpty(userNameProp)) {
                emailProp = '-';
            } else if (isEmpty(emailProp) && !isEmpty(userNameProp)) {
                emailProp = userNameProp;
            }

            try {
                if (JSON.parse(avatarProp)) {
                    userAvatarProp = JSON.parse(avatarProp);
                } else {
                    userAvatarProp = R.images.AVATAR_01;
                }
            } catch (error) {
                userAvatarProp = R.images.AVATAR_01;
            }

            if (state.fullName !== fullNameProp || state.email !== emailProp || state.userAvatar !== userAvatarProp) {
                return Object.assign({}, state, { fullName: fullNameProp, email: emailProp, userAvatar: userAvatarProp });
            }
