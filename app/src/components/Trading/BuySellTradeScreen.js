...
import { getCurrencyBalance } from '../../actions/Trade/TradeBalanceActions';
...
class BuySellTradeScreen extends Component {
    ...
    async componentDidMount() {
        ...
        this.listenerBuySellPairData = addListener(Events.BuySellPairData, async (result) => {
            ...
            if (await isInternet()) {
                ...
                // Call Pair Rates API
                // this.props.getPairRates({ PairName: result.PairName });

                //To get balances
                this.props.getBalance();

                // Call BuyerBooklist api
                this.props.fetchBuyerBookList({ Pair: result.PairName });

                // Call SellerBooklist api
                this.props.fetchSellerBookList({ Pair: result.PairName });
                ...
                if (marketList) {
                    ...
                    if (state.marketList == null || (state.marketList != null && marketList !== state.marketList)) {
                        ...
                        if (validateResponseNew({ response: marketList, isList: true })) {
                            ...
                            return Object.assign({}, state, { marketList })
                        }
                    }
                }
                ...
                if (buySellTrade) {
                    ...
                    if (state.buySellTrade == null || (state.buySellTrade != null && buySellTrade !== state.buySellTrade)) {
                        ...
                        try {
                            ...
                            return Object.assign({}, state, { buySellTrade })
                        }
                    }
                }
            } catch (error) {
    ...
    componentDidUpdate = async (prevProps, prevState) => {
        ...
        if (this.state.buySellTrade && this.state.redirectToSuccess) 
        {
            ...
            this.props.navigation.navigate('BuySellTradeSuccess', { item: params });
    ...
    async callAPIs() {
        ...
        if (await isInternet()) {
            ...
            // Call BuyerBooklist api
            this.props.fetchBuyerBookList({ Pair: this.state.result ? this.state.result.PairName : AppConfig.initialPair });

            // Call SellerBooklist api
            this.props.fetchSellerBookList({ Pair: this.state.result ? this.state.result.PairName : AppConfig.initialPair });
    ...
    async callDependencyAPIS() {
        ...
        if (await isInternet()) {
            ...
            // Don't Delete These Code it will be use in future if condition needed with minPrice and maxPrice
            // Call Pair Rates API
            // this.props.getPairRates({ PairName: pairRecord.PairName });

            // Call Market Cap Detail API
            this.props.fetchMarketCap({ Pair: pairRecord.PairName ? pairRecord.PairName : AppConfig.initialPair });
        }
    }
    ...
    render() {
        ...
        return (
            <SafeView style={{
                ...
                {/* Custom Toast */}
                <CommonToast isMainScreen={true} ref={(cmp) => this.displayToast = cmp} />
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        getBalance: () => dispatch(getCurrencyBalance()),
...
export default connect(mapStatToProps, mapDispatchToProps)(BuySellTradeScreen);