    ...
    render() {
        ...
        if (this.state.tickers && this.state.tickers.length > 0) {
            return <ScrollView
                ...
                {
                    (this.state.tickers ? this.state.tickers : []).map((item) => <MarketTickerItem key={item.PairId.toString()} width={this.state.itemWidth} item={item} preference={this.props.preference} />)
                }
            </ScrollView>;
        } else {
            return <View></View>;
