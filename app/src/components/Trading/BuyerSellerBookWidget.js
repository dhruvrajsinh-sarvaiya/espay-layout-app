...
import { Method, Constants, Events, ServiceUtilConstant } from '../../controllers/Constants';
...
class BuyerSellerBookWidget extends Component {
	...
	async componentDidMount() {
		...
		if (this.state.isDetail && await isInternet()) {
			...
			if (!isFetchingBuyerBook) {
				...
				this.props.fetchBuyerBookList({ Pair: this.props.PairName ? this.props.PairName : AppConfig.initialPair, IsMargin: this.isMargin ? 1 : 0 });
			}

			if (!isFetchingSellerBook) {
				// Call SellerBooklist api
				this.props.fetchSellerBookList({ Pair: this.props.PairName ? this.props.PairName : AppConfig.initialPair, IsMargin: this.isMargin ? 1 : 0 });
			}
		}
		...
		this.listenerReceiveBulkSellerBook = addListener(Method.ReceiveBulkSellerBook, (receivedMessage) => {
			...
			try {
				...
				if ((response.EventTime && this.state.socketLPSellData.length == 0) || (this.state.socketLPSellData.length !== 0 && response.EventTime >= this.state.socketLPSellData.EventTime)) {
					if (this.props.PairName === response.Parameter && typeof response.IsMargin !== 'undefined' && response.IsMargin === (this.isMargin ? 1 : 0)) {
						let LPSellerBook = this.state[this.isMargin ? 'LPMarginSellerBook' : 'LPSellerBook'];
						...
						this.setState({ [this.isMargin ? 'LPMarginSellerBook' : 'LPSellerBook']: LPSellerBook, socketLPSellData: response, sellerBookList: res })
					}
				}
		...
		if ((props.shouldDisplay && isCurrentScreen(props)) || isCurrentScreen(props)) {
			...
			try {
				...
				if (marketCap) {
					...
					let stateMC = state[getData(ServiceUtilConstant.KEY_IsMargin) ? 'marginMarketCap' : 'marketCap'];

					//if marketCap is null or old and new data of tradeHistory  and response is different then validate it
					if (stateMC == null || (stateMC != null && marketCap !== stateMC)) {
						validateResponseNew({ response: marketCap, isList: true })

						if (marketCap.response != null) {
							return Object.assign({}, state, {
								[getData(ServiceUtilConstant.KEY_IsMargin) ? 'marginMarketCap' : 'marketCap']: marketCap,
				...
				if (buyerBookData) {
					...
					let stateBBD = state[getData(ServiceUtilConstant.KEY_IsMargin) ? 'marginBuyerBookData' : 'buyerBookData'];
					...
					if (stateBBD == null || (stateBBD != null && buyerBookData !== stateBBD)) {
						...
						if (validateResponseNew({ response: buyerBookData, isList: true })) {
							...
							return Object.assign({}, state, {
								[getData(ServiceUtilConstant.KEY_IsMargin) ? 'marginBuyerBookData' : 'buyerBookData']: buyerBookData,
				...
				if (sellerBookData) {
					...
					let stateSBD = state[getData(ServiceUtilConstant.KEY_IsMargin) ? 'marginSellerBookData' : 'sellerBookData'];
					...
					if (stateSBD == null || (stateSBD != null && sellerBookData !== stateSBD)) {
						...
						if (validateResponseNew({ response: sellerBookData, isList: true })) {
							...
							return Object.assign({}, state, {
								[getData(ServiceUtilConstant.KEY_IsMargin) ? 'marginSellerBookData' : 'sellerBookData']: sellerBookData,
	...
	onPressSetSellData(item) {
		try {
			sendEvent(Events[this.isMargin ? 'BuySellInputMargin' : 'BuySellInput'], { module: Constants.TradeTypes.Sell, ...item });
		} catch (e) {
			//handle catch
		}
	}
	...
	onPressSetBuyData(item) {
		try {
			sendEvent(Events[this.isMargin ? 'BuySellInputMargin' : 'BuySellInput'], { module: Constants.TradeTypes.Buy, ...item });
		} catch (e) {
			//handle catch
		}
	}
