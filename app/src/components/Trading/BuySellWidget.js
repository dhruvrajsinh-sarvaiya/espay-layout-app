...
class BuySellWidget extends Component {
    ...
    async componentDidMount() {
        ...
        this.listenerBuySellPairData = addListener(Events.BuySellPairData, async (result) => {
        ...
        this.listenerBuySellInput = addListener(Events.BuySellInput, (item) => {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (JSON.stringify(props.preference[ServiceUtilConstant.KEY_CurrencyPair]) !== JSON.stringify(state.result)) {
            return Object.assign({}, state, {
                result: props.preference[ServiceUtilConstant.KEY_CurrencyPair], wallets: null,
            });
        }
        ...
        if (props.shouldDisplay && isCurrentScreen(props)) {
            ...
            try {
                ...
                const { buySellTradeData: { buySellTrade }, pairRatesData: { pairRates }, wallets: { wallets }, pairRecord, marketCapData: { marketCap } } = props;
                ...
                if (pairRates) {
                    ...
                    if (state.pairRates == null || (state.pairRates != null && pairRates !== state.pairRates)) {
                        ...
                        if (validateResponseNew({ response: pairRates, isList: true })) {
                            ...
                            return Object.assign({}, state, { pairRates })
                        }
                    }
                }
                ...
                if (wallets) {
                    ...
                    if (state.wallets == null || (state.wallets != null && wallets !== state.wallets)) {

                        //Validate Response and Check balance if Pair is available in response
                        if (validateResponseNew({ response: wallets.Wallets, isList: true })) {
    ...
    async onBuySell() {
        ...
        if (await isInternet()) {
            ...
            if (this.state.module == 4) { this.props.updateBuyParams(params); }

            if (this.state.module == 5) { this.props.updateSellParams(params); }
                    ...
                    <View style={{ flex: 1, flexDirection: 'column', }} >
                        ...
                        <View style={{ flexDirection: 'column', paddingLeft: R.dimens.WidgetPadding, paddingRight: R.dimens.WidgetPadding }}>
                            ...
                            <View style={{ paddingTop: R.dimens.WidgetPadding, paddingBottom: isMarketOrder ? R.dimens.widgetMargin : R.dimens.WidgetPadding, }}>
                                ...
                                {
                                    isMarketOrder &&
                                    <CardView cardBackground={R.colors.bottomMenuBackground} style={{ marginBottom: R.dimens.widget_top_bottom_margin, }} cardElevation={0} >
                            ...
                            <View style={{ marginTop: R.dimens.widget_top_bottom_margin, flexDirection: 'row', justifyContent: 'space-between', }}>
                                ...
                                <TextViewHML style={{ flex: 1, fontSize: R.dimens.smallText, textAlign: 'right', color: R.colors.textPrimary, marginLeft: R.dimens.widgetMargin, marginRight: R.dimens.widgetMargin }}>{finalTotal ? parseFloatVal(finalTotal).toFixed(8) : parseFloatVal('0.0').toFixed(8)}</TextViewHML>
                            </View>
    ...
    try {
        ...
        if (pair) {
            ...
            if (wallets.Wallets && wallets.Wallets.length > 0) {
                ...
                let firstCoinIndex = wallets.Wallets.findIndex(item => (item.CoinName === firstCoin && item.IsDefaultWallet == 1));
                let secondCoinIndex = wallets.Wallets.findIndex(item => (item.CoinName === secondCoin && item.IsDefaultWallet == 1));

                // get balances for both currency
                let firstCoinBalance = firstCoinIndex > -1 ? wallets.Wallets[firstCoinIndex].Balance : 0;
                let secondCoinBalance = secondCoinIndex > -1 ? wallets.Wallets[secondCoinIndex].Balance : 0;
                ...
                if (firstCoinIndex > -1) {
                    if (module == 4) {
                        creditWalletId = wallets.Wallets[firstCoinIndex].AccWalletID;
                    } else {
                        debitWalletId = wallets.Wallets[firstCoinIndex].AccWalletID;
                    }
                }
                ...
                if (secondCoinIndex > -1) {
                    if (module == 4) {
                        debitWalletId = wallets.Wallets[secondCoinIndex].AccWalletID;
                    } else {
                        creditWalletId = wallets.Wallets[secondCoinIndex].AccWalletID;
                    }
                }
