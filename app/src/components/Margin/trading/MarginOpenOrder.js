...
import { View, FlatList, TouchableWithoutFeedback, RefreshControl, Easing } from 'react-native';
...
class MarginOpenOrder extends Component {
    ...
    async componentDidMount() {
        ...
        if (await isInternet()) {
            ...
            if (this.props.isWidget === undefined || (this.props.isWidget !== undefined && this.props.isWidget == false)) {
                ...
                this.request = {
                    IsMargin: 1 // for get Open Order Based On Margin  
                }
            ...
            try {
                if (JSON.parse(receivedMessage).Data) {
                    ...
                    if ((signalData.EventTime && this.state.socketData.length === 0) || (this.state.socketData.length !== 0 && signalData.EventTime >= this.state.socketData.EventTime)) {
                        ...
                        if (typeof signalData.IsMargin !== 'undefined' && signalData.IsMargin === 1) {
                            ...
                            if (parseFloatVal(newData.Price) !== 0) {
                                ...
                                if (findIndexOrderId === -1) {
                                    ...
                                    if (parseFloatVal(newData.Amount) !== 0) {
                                        ...
                                        marginOpenOrder.push(newData);
                                        latestOrderList.map((value, key) => {
                                            marginOpenOrder.push(value)
                                        })
                                    }
                                } else {
                                    ...
                                    if (parseFloatVal(newData.Amount) > 0) {
                                        ...
                                        marginOpenOrder = latestOrderList;
                                    } else {
                                ...
                                if (parseFloatVal(newData.Amount) == 0) {
                                    ...
                                    marginOpenOrder.push(newData);
                                    latestOrderList.map((value, _key) => {
                                        marginOpenOrder.push(value)
                                    })
                                }
                            }
                            ...
                            let sortedArray = arraySort(marginOpenOrder, 'TrnDate', { reverse: true });
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        if (this.props.preference.theme !== nextProps.preference.theme
            ...
            if (this.props.isWidget === undefined && this.props.shouldDisplay === undefined) {

                // stop twice api call
                return isCurrentScreen(nextProps);
            } else if ((this.props.isWidget !== undefined && this.props.isWidget) && (this.props.shouldDisplay !== undefined && this.props.shouldDisplay)) {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        //if component's isWidget & shouldDisplay both props are missing then display open order
        if (props.isWidget === undefined && props.shouldDisplay === undefined) {
        ...
        else if ((props.isWidget !== undefined && props.isWidget) && (props.shouldDisplay !== undefined && props.shouldDisplay)) {
            displayCondition = true;
        } else {
        ...
        if (MarginOpenOrder.oldProps !== props) {
            MarginOpenOrder.oldProps = props;
        } else {
            return null;
        }
        ...
        if (displayCondition && isCurrentScreen(props)) {
            ...
            try {
                ...
                if (openOrder) {
                    ...
                    if (state.marginOpenOrder == null || (state.marginOpenOrder != null && openOrder !== state.marginOpenOrder)) {
                        ...
                        if (validateResponseNew({ response: openOrder, isList: true })) {
                            ...
                            return Object.assign({}, state, {
                                marginOpenOrder: openOrder,
                                response: finalRecords,
                                refreshing: false,
                            })
                        } else {
                            return Object.assign({}, state, { marginOpenOrder: openOrder, response: [], refreshing: false });
                        }
                    }
                }
    ...
    async onCancelPress() {
        ...
        if (await isInternet()) {
            ...
            this.props.cancelOpenOrder({ TranNo: this.state.cancelItem.Id, CancelAll: 0, OrderType: 0, IsMargin: 1 })
        }
    }
    ...
    handleOptionMenu(i) {
        ...
        if (marginDialogShowCount == 0) {
            ...
            if (this.state.response.length > 0 && isMarginOrderType) {

                let marginMessage;

                switch (i) {
                    case 0: marginMessage = R.strings.cancelAllMessage; break;
                    case 1: marginMessage = R.strings.formatString(R.strings.cancelMessage, { type: R.strings.limit }); break;
                    case 2: marginMessage = R.strings.formatString(R.strings.cancelMessage, { type: R.strings.market }); break;
                    case 3: marginMessage = R.strings.formatString(R.strings.cancelMessage, { type: R.strings.stopLimit }); break;
                    default: marginMessage = R.strings.cancelAllMessage; break;
                }
                this.showCancelDialog(marginMessage, true, marginOrderType);
            } else {
    ...
    showCancelDialog(message, requestCall = false, OrderType = 0) {
        if (requestCall) {
            ...
            showAlert(R.strings.cancelOrder, message, 3, async () => {

                //Check NetWork is Available or not
                if (await isInternet()) {

                    //Request to Cancel All Orders
                    let request = { TranNo: 0, CancelAll: OrderType == 0 ? 1 : 2, OrderType: OrderType, IsMargin: 1 };
            ...
            //show cancel order dialog with appropriate message.
            showAlert(R.strings.cancelOrder, message, 3);
        }
    }
    ...
    async onHideTogglePress() {
        ...
        if (await isInternet()) {
            ...
            this.request = {
                ...this.request,
                pairName: this.state.isToggle ? this.state.pairName : '',
                IsMargin: 1 // for get Open Order Based On Margin  
            }
    ...
    async onResetPress() {
        ...
        this.setState({ stFromDate: getCurrentDate(), stToDate: getCurrentDate(), selectedCurrencyPair: R.strings.all, selectedType: R.strings.all })
    ...
    render() {
        ...
        let finalItems;
        let finalItem = this.state.isFilter ? this.state.filterResponse : this.state.response;
        if (finalItem) {
            ...
            return (<View>
                ...
                <View style={{
                    ...
                    {finalItems.length > 0 && <ImageButton
                        ...
                        onPress={() => this.props.navigation.navigate('MarginOpenOrder')}
    ...
    cancelOrderDialog() {
        return (<AlertDialog
            ...
            {this.state.cancelItem && <View style={{ justifyContent: 'space-between' }}>
                ...
                <TextViewHML style={{ fontSize: R.dimens.smallText, color: R.colors.textPrimary, marginTop: R.dimens.widget_top_bottom_margin }}>
    ...
    renderHeader(props) {
        ...
        return (
            <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
                <Separator width={R.dimens.statusIndicatorWidth} height={R.dimens.largeText} style={{ marginLeft: 0, marginRight: R.dimens.widgetMargin }} color={color} />
                <TextViewMR style={{ width: '70%', color: R.colors.textPrimary, fontSize: R.dimens.smallText }}>{props.module}</TextViewMR>
                <TextViewMR style={{ width: '30%', textAlign: 'right', color: R.colors.textPrimary, fontSize: R.dimens.smallText }}>{props.pairName}</TextViewMR>
            </View>
        )
    }
                ...
                <View style={{
                    ...
                    <CardView style={{
                        ...
                        <View style={{ flexDirection: 'row', }}>
                            <TextViewMR style={{ flex: 1, color: R.colors.textPrimary, fontSize: R.dimens.smallText }}>
                                {item.PairName.replace('_', '/')}
                                {<TextViewHML style={{ color: fontColor, fontSize: R.dimens.smallText, fontWeight: 'normal' }}> {item.Type}</TextViewHML>}
                                {<TextViewHML style={{ color: fontColor, fontSize: R.dimens.smallText, fontWeight: 'normal' }}> - {item.OrderType}</TextViewHML>}
                            </TextViewMR>
                            <TextViewHML style={{ color: R.colors.textSecondary, fontSize: R.dimens.smallestText, textAlign: 'right', alignSelf: 'flex-end' }}>{convertDateTime(item.TrnDate, 'YYYY-MM-DD HH:mm:ss', false)}</TextViewHML>
                        </View>
                        ...
                        <View style={{ flexDirection: 'row', marginTop: R.dimens.widgetMargin }}>
                            ...
                            <View style={{ flexDirection: 'row', width: '70%', }}>
                                <View style={{ flexDirection: 'row' }}>
                                    ...
                                    <View>
                                        <TextViewHML style={{ fontSize: R.dimens.volumeText, color: R.colors.textPrimary }}>{amount} {item.Order_Currency}</TextViewHML>
                                        <TextViewHML style={{ fontSize: R.dimens.volumeText, color: R.colors.textPrimary, }}>{item.OrderType.toLowerCase().includes('market') ? R.strings.market : (price + ' ' + item.Delivery_Currency)}</TextViewHML>
                                    </View>
                                </View>
                            </View>
                            ...
                            <View style={{ width: '30%', justifyContent: 'flex-end' }}>
                                <TouchableWithoutFeedback onPress={props.onPress}>
                                    <View style={{ alignSelf: 'flex-end', }}>
                                        <TextViewMR style={this.styles().cancelbuttonText}>{R.strings.cancel}</TextViewMR>
                                    </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(MarginOpenOrder);