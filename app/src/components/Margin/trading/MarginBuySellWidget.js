...
class MarginBuySellWidget extends Component {
    ...
    async componentDidMount() {
        ...
        this.listenerBuySellPairData = addListener(Events.BuySellPairDataMargin, async (result) => {
        ...
        this.listenerRecieveMarginWalletBal = addListener(Method.RecieveWalletBal, (receivedMessage) => {

            // check for current screen
            if (isCurrentScreen(this.props)) {
                try {
                    let marginResponse = JSON.parse(receivedMessage);

                    if (marginResponse && marginResponse.Data) {

                        if ((marginResponse.EventTime && this.state.socketBuyData.length == 0) ||
                            (this.state.socketBuyData.length !== 0 && marginResponse.EventTime >= this.state.socketBuyData.EventTime)) {

                            if (marginResponse.Data.CoinName !== '') {
                                ...
                                let state = {}
                                if (this.state.module == 4 && marginResponse.Data.CoinName === secondCurrency) {
                                    state = { secondBalance: marginResponse.Data.Balance };
                                }

                                if (this.state.module == 5 && marginResponse.Data.CoinName === firstCurrency) {
                                    state = { firstBalance: marginResponse.Data.Balance };
                                }

                                this.setState({ ...state, socketBuyData: marginResponse })
                            }
        ...
        this.listenerBuySellInput = addListener(Events.BuySellInputMargin, (item) => {
    ...
    componentWillUnmount() {
        ...
        if (this.listenerRecieveMarginWalletBal) {
            this.listenerRecieveMarginWalletBal.remove();
        }
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (props.shouldDisplay && isCurrentScreen(props)) {
            ...
            try {
                ...
                if (state.marginType != props.orderType) {
                    return Object.assign({}, state, {
                        marginType: props.orderType,
                        amount: ''
                    })
                }

                //Get All Updated field of Particular actions
                const { buySellTradeData: { buySellTrade }, pairRatesData: { pairRates }, wallets: { marginWallets }, pairRecord, marketCapData: { marketCap } } = props;
                ...
                if (marginWallets) {
                    ...
                    if (state.marginWallets == null || (state.marginWallets != null && marginWallets !== state.marginWallets)) {

                        //Validate Response and Check balance if Pair is available in response
                        if (validateResponseNew({ response: marginWallets, isList: true })) {
    ...
    onIncDec(item, isIncrement) {
        ...
        let buyPrice = (this.state.marginType == Constants.OrderTypes.MARKET_ORDER) ? this.state.buyPriceOriginal : this.state.buyPrice;
        let sellPrice = (this.state.marginType == Constants.OrderTypes.MARKET_ORDER) ? this.state.sellPriceOriginal : this.state.sellPrice;
        ...
        switch (item) {
            ...
            case R.strings.Amount:
                ...
                if (this.state.marginType == Constants.OrderTypes.LIMIT_ORDER || this.state.marginType == Constants.OrderTypes.STOP_LIMIT_ORDER) {
                    total = amount * lastPrice;
                }
    ...
    async onBuySell() {
       ...
       sellPrice = isEmpty(sellPrice) ? '0' : sellPrice; */
        ...
        if (this.state.marginType == Constants.OrderTypes.STOP_LIMIT_ORDER) {
        ...
        if (isEmpty(lastPrice) && this.state.marginType != Constants.OrderTypes.MARKET_ORDER) {

            let toastMsg = this.state.marginType == Constants.OrderTypes.STOP_LIMIT_ORDER ? R.strings.LimitNotEmpty : R.strings.PriceNotEmpty;
        ...
        if (parseFloatVal(lastPrice) <= 0 && this.state.marginType != Constants.OrderTypes.MARKET_ORDER) {
            let toastMsg = this.state.marginType == Constants.OrderTypes.STOP_LIMIT_ORDER ? R.strings.LimitGreaterThanZero : R.strings.PriceGreaterThanZero;
            this.props.Toast(toastMsg)
            return;
        }
        ...
        //if price is less then min price then show msg
        if (lastPrice < minPrice) {
            this.props.Toast(R.strings.MinimumBidPrice + minPrice)
            return;
        }

        //if price is greather then or equal to max price then show msg
        if (lastPrice >= maxPrice) {
            this.props.Toast(R.strings.MaximumBidPrice + maxPrice)
            return;
        } */
        ...
        let params = {
            ...
            OrderType: this.state.marginType,
    ...
    render() {
        ...
        let isLimitOrSpotOrder = this.state.marginType == Constants.OrderTypes.LIMIT_ORDER || this.state.type == Constants.OrderTypes.SPOT_ORDER;
        let isMarketOrder = this.state.marginType == Constants.OrderTypes.MARKET_ORDER;
        let isStopLimitOrder = this.state.marginType == Constants.OrderTypes.STOP_LIMIT_ORDER;
                        ...
                        <View style={{ flexDirection: 'column', paddingLeft: R.dimens.WidgetPadding, paddingRight: R.dimens.WidgetPadding, }}>
                            ...
                            <View style={{ paddingTop: R.dimens.WidgetPadding, paddingBottom: isMarketOrder ? R.dimens.widgetMargin : R.dimens.WidgetPadding }}>
                                ...
                                }
                                {
                                    // DO NOT DELETE
                                    /* !isMarketOrder &&
                                    <TextViewHML style={{ fontSize: R.dimens.smallestText, color: R.colors.textSecondary, alignSelf: 'flex-end', marginTop: R.dimens.widget_top_bottom_margin }}>=0.263444444 USD</TextViewHML> */
                                }
                            </View>
...
export default connect(mapStatToProps, mapDispatchToProps)(MarginBuySellWidget);
...
function findBalance(props, pair, moduleName) {
    ...
    try {
        ...
        let { wallets: { marginWallets } } = props;

        // if pair is not null or undefined than get balance
        if (pair) {

            //Check if listCoin has Wallets
            if (marginWallets && marginWallets.Data && marginWallets.Data.length > 0) {
                ...
                let firstCoinIndex = marginWallets.Data.findIndex(item => (item.CoinName === firstCoin && item.IsDefaultWallet == 1));
                let secondCoinIndex = marginWallets.Data.findIndex(item => (item.CoinName === secondCoin && item.IsDefaultWallet == 1));

                // get balances for both currency
                let firstCoinBalance = firstCoinIndex > -1 ? marginWallets.Data[firstCoinIndex].Balance : 0;
                let secondCoinBalance = secondCoinIndex > -1 ? marginWallets.Data[secondCoinIndex].Balance : 0;
                ...
                if (firstCoinIndex > -1) {
                    if (moduleName == 4) {
                        creditWalletId = marginWallets.Data[firstCoinIndex].AccWalletID;
                    } else {
                        debitWalletId = marginWallets.Data[firstCoinIndex].AccWalletID;
                    }
                }
                ...
                if (secondCoinIndex > -1) {
                    if (moduleName == 4) {
                        debitWalletId = marginWallets.Data[secondCoinIndex].AccWalletID;
                    } else {
                        creditWalletId = marginWallets.Data[secondCoinIndex].AccWalletID;
                    }
                }
