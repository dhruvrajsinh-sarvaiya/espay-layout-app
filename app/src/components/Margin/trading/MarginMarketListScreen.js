...
import { storeBaseCurrency, onFetchMarginMarkets } from '../../../actions/Trade/TradeActions';
...
const CacheName = 'MarginMarketListScreen';

class MarginMarketListScreen extends Component {
    ...
    async componentDidMount() {
        ...
        if (await isInternet()) {
            ...
            if (this.props.favourites.marginFavouriteList === null) {
                //Call API To get the favourites list
                this.props.getFavourites({ IsMargin: 1 });
            }

            if (this.props.marketData.marginMarketList === null) {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        try {
            ...
            let { marketData: { marginMarketList }, favourites: { marginFavouriteList } } = props;

            // check market list data is available or not
            if (marginMarketList) {

                //if local marginMarketList state is null or its not null and also different then new response then and only then validate response.
                if (state.marginMarketList == null || (state.marginMarketList != null && marginMarketList !== state.marginMarketList)) {
                    let newState = { marginMarketList };

                    if (validateResponseNew({ response: marginMarketList, isList: true })) {

                        //To check if there is list then loop through all records and find common second names
                        if (marginMarketList.response) {

                            //Loop through all records
                            marginMarketList.response.map((marketItem) => {
                            ...
                            newState = Object.assign({}, newState, { marketListData: marginMarketList.response });
                        }
                    }
            ...
            if (marginFavouriteList) {

                //if local marginFavouriteList state is null or its not null and also different then new response then and only then validate response.
                if (state.marginFavouriteList == null || (state.marginFavouriteList != null && marginFavouriteList !== state.marginFavouriteList)) {
                    let newState = { marginFavouriteList };

                    //if marginFavouriteList response is success then store array list else store empty list
                    if (validateResponseNew({ response: marginFavouriteList, isList: true })) {
                        let res = parseArray(marginFavouriteList.response);
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        if (this.props.preference.theme !== nextProps.preference.theme
            ...
            || this.state.marginFavouriteList !== nextProps.favourites.marginFavouriteList
            || this.props.favourites.isFetching !== nextProps.favourites.isFetching
            || this.state.width !== nextState.width) {
            return true;
        } else {
            if (this.state.marginMarketList !== nextProps.marketData.marginMarketList ||
    ...
    render() {
        ...
        return (
            <SafeView style={{ flex: 1, backgroundColor: R.colors.background, }}>
                ...
                {!this.props.marketData.isFetchMarket && this.state.marginMarketList === null && <ListEmptyComponent />}

                {!this.props.marketData.isFetchMarket && this.props.marketData.marginMarketList &&
                    <IndicatorViewPager
                        ref='MarketTabs'
                            ...
                            <View key={tabItem}>
                                ...
                                <FlatList
                                    ...
                                    renderItem={({ item }) => <MarketPairItem
                                        ...
                                        onPress={() => this.onTradeHistoryItemPress(item)} />}
                                    keyExtractor={item => item.PairId.toString()}
                                    contentContainerStyle={contentContainerStyle(this.props.marketData.marginMarketList.response)}
                                    ListEmptyComponent={<ListEmptyComponent />}
                                />
                            </View>)}
                    </IndicatorViewPager>
        ...
        return (
            <AnimatableItem>
                <TouchableWithoutFeedback onPress={this.props.onPress}>
                    ...
                    <View style={[this.styles().simpleItem, { marginBottom: R.dimens.widgetMargin, borderRadius: R.dimens.cardBorderRadius, }]}>
                        <View style={[this.styles().simpleItem, { width: '100%', marginTop: 0 }]}>
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        getPairList: (payload) => dispatch(onFetchMarginMarkets(payload)),
...
export default connect(mapStatToProps, mapDispatchToProps)(MarginMarketListScreen);