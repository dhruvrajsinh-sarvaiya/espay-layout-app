...
class MarginOrderManagementScreen extends Component {
    ...
    async componentDidMount() {
        ...
        this.listenerRecieveTradeHistory = addListener(Method.RecieveTradeHistory, (marginReceivedMessage) => {

            // check for current screen
            if (isCurrentScreen(this.props)) {
                try {
                    let response = JSON.parse(marginReceivedMessage);

                    //if connection has data
                    if (response) {
                        let marginList = this.state.response;

                        //if list is empty then add record
                        if (marginList.length == 0) {
                            marginList.push(response.Data);
                        }
                        ...
                        if (marginList.length > 0) {
                            ...
                            let indexOfPrice = marginList.findIndex(el => el.TrnNo == response.Data.TrnNo);

                            //if same price record found then check for its existing amount
                            if (indexOfPrice > -1) {

                                //Updating records
                                marginList[indexOfPrice] = response.Data
                        ...
                        let marginSortedArray = arraySort(marginList, 'DateTime', { reverse: true });

                        if (this.state.isShowHistory) {
                            this.setState({ response: marginSortedArray });
                        }
                    }
                } catch (_error) {
                    //parsing error
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (MarginOrderManagementScreen.oldProps !== props) {
            MarginOrderManagementScreen.oldProps = props;
        } else {
            return null;
        }
                ...
                try {
                    ...
                    if (state.marketListFilter == null || (state.marketListFilter != null && marketListFilter !== state.marketListFilter)) {
                        ...
                        if (validateResponseNew({ response: marketListFilter, isList: true })) {
                            let marginRes = parseArray(marketListFilter.response);

                            let marginPairList = [{ value: R.strings.Please_Select },];

                            //to check if records are exist
                            if (marginRes && marginRes.length > 0) {

                                //Base Currency Loop
                                marginRes.map((baseCurency) => {

                                    baseCurency.PairList.map(item => {
                                        if (marginPairList.every(el => el !== item.PairName)) {
                                            marginPairList.push({ value: item.PairName });
                                        }
                                    })
                                })
                            }
                            return Object.assign({}, state, {
                                CurrencyPair: marginPairList,
                                marketListFilter
                            })
                        } else {
                            return Object.assign({}, state, {
...
function mapStatToProps(state) {
    ...
    return {
        result: { ...state.orderHistoryReducer, ...state.marginTradeReducer },
    }
}
...
export default connect(mapStatToProps, mapDispatchToProps)(MarginOrderManagementScreen);