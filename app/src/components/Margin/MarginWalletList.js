...
class MarginWalletList extends Component {
    ...
    _onCompletePress = async () => {
        ...
        if (await isInternet()) {
            ...
            let walletUsageType = ''
            if (this.state.selectedWalletType === this.state.walletTypeItems[1].value) { walletUsageType = 5 }
            else if (this.state.selectedWalletType === this.state.walletTypeItems[2].value) { walletUsageType = 6 }
            else if (this.state.selectedWalletType === this.state.walletTypeItems[3].value) { walletUsageType = 7 }

            let status = ''
            if (this.state.selectedStatus === this.state.statusItems[1].value) { status = 1 }
            else if (this.state.selectedStatus === this.state.statusItems[2].value) { status = 0 }
