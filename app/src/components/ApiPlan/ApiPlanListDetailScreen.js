						...
						<View>
							<ScrollView showsVerticalScrollIndicator={false}>
								<View style={{ flex: 1, padding: R.dimens.margin_left_right, }}>
									...
									{/* Get Current Coin and their balance */}
									<CurrentCoinAndBal currentBal={this.state.currentBal} currentCoin={this.state.currentCoin} />

									{/* PlanName and Active/Inactive Status */}
									<View style={{ flexDirection: 'row', alignItems: 'center', }}>
										<TextViewMR style={{ flex: 1, fontSize: R.dimens.mediumText, color: R.colors.textPrimary, }}>{item.PlanName}</TextViewMR>
									</View>

									{/* Price and PlanValidity */}
									<View style={{ flexDirection: 'row', alignItems: 'center', marginTop: R.dimens.margin }}>
									...
									<ScrollView showsVerticalScrollIndicator={false}>
										<View style={{ marginTop: R.dimens.margin, justifyContent: 'center' }}>
											...
											<PlanSubItem value={item.ConcurrentEndPoints} title={R.strings.concurrentIpAddressLimit} />
											<PlanSubItem value={item.MaxReqSize} title={R.strings.maxRequestSize} />
											<PlanSubItem value={item.MaxResSize} title={R.strings.maxResponseSize} />
											<PlanSubItem value={item.HistoricalDataMonth} title={R.strings.historicalData} />

											<View style={{ flex: 1, flexDirection: 'row' }}>
												<TextViewHML style={{ flex: 1, color: R.colors.textSecondary, fontSize: R.dimens.smallestText }}>{R.strings.recursive}</TextViewHML>
												<ImageTextButton
													icon={item.IsPlanRecursive == 1 ? R.images.IC_CHECKMARK : R.images.IC_CANCEL}
													style={{ margin: 0, flex: 1 }}
													iconStyle={{ height: R.dimens.titleIconHeightWidth, width: R.dimens.titleIconHeightWidth, tintColor: item.IsPlanRecursive == 1 ? R.colors.successGreen : R.colors.failRed }}
												/>
											</View>
							...
							<View style={{ flex: 1, padding: R.dimens.margin_left_right, }}>
...

								{
									(item.ReadOnlyAPI != null && Object.values(item.ReadOnlyAPI).length > 0 || item.FullAccessAPI != null && Object.values(item.FullAccessAPI).length > 0) ?
										<View style={{ flex: 1, }}>
											<ScrollView showsVerticalScrollIndicator={false}>
												...
												{/* Read Only Method List Title */}
												{
													...
													<View style={{ marginTop: R.dimens.margin }}>
														<Text style={{ color: R.colors.textSecondary, fontFamily: Fonts.MontserratSemiBold, fontSize: R.dimens.smallText, }}>{R.strings.readOnly}</Text>
														<Separator style={{ marginBottom: R.dimens.widgetMargin, marginTop: R.dimens.widgetMargin, marginLeft: 0, marginRight: 0 }} />
													</View>
												}
												...
												<View>
													{
														item.ReadOnlyAPI != null && Object.values(item.ReadOnlyAPI).map((itemReadOnly, index) =>
															<View style={{ flexDirection: 'row', }} key={index.toString()}>
																<TextViewHML style={{ fontSize: R.dimens.smallestText, flex: 1, color: R.colors.textSecondary }}>{itemReadOnly}</TextViewHML>
																<ImageTextButton
																	icon={R.images.IC_CHECK_CIRCLE}
																	style={{ margin: 0 }}
																	iconStyle={{ height: R.dimens.titleIconHeightWidth, width: R.dimens.titleIconHeightWidth, tintColor: R.colors.successGreen }}
																/>
															</View>
														)
													}
												...
												{/* Full Access Method List Title */}
												{
													...
													<View style={{ marginTop: R.dimens.margin_top_bottom }}>
														...
														<Separator style={{ marginTop: R.dimens.widgetMargin, marginBottom: R.dimens.widgetMargin, marginRight: 0, marginLeft: 0, }} />
													</View>
												}
												...
												<View>
													{
														item.FullAccessAPI != null && Object.values(item.FullAccessAPI).map((itemFullAccess, index) =>
															<View style={{ flexDirection: 'row', }} key={index.toString()}>
																<TextViewHML style={{ fontSize: R.dimens.smallestText, flex: 1, color: R.colors.textSecondary, }}>{itemFullAccess}</TextViewHML>
																<ImageTextButton
																	style={{ margin: 0 }}
																	iconStyle={{ tintColor: R.colors.successGreen, width: R.dimens.titleIconHeightWidth, height: R.dimens.titleIconHeightWidth, }}
																	icon={R.images.IC_CHECK_CIRCLE}
																/>
															</View>
														)
										...
										:
										<ListEmptyComponent
											message={R.strings.noApiMethodRequested} />
								}
							</View>
						</View>
...
export class CurrentCoinAndBal extends Component {
	...
	shouldComponentUpdate(nextProps, _nextState) {
		...
		if (this.props.currentCoin !== nextProps.currentCoin || this.props.currentBal !== nextProps.currentBal) { return true }
		return false
	}

	render() {
		let { currentBal, currentCoin, } = this.props
		return (
			<View style={{ alignItems: 'center', flexDirection: 'row', justifyContent: 'flex-end' }}>
				<TextViewHML style={{ fontSize: R.dimens.smallestText, color: R.colors.textSecondary, }}>{R.strings.Ava_Bal}</TextViewHML>
				<Text style={{ color: R.colors.textPrimary, marginLeft: R.dimens.widgetMargin, fontSize: R.dimens.smallestText, fontFamily: Fonts.MontserratSemiBold }}>{currentBal.toFixed(8) + ' ' + currentCoin}</Text>
			</View>
		)
	}
}
