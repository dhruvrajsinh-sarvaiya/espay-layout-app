...
class AddCustomLimitsForKeys extends Component {
	...
	onSetCustomLimits = async () => {
		...
		if (isEmpty(this.state.MaxCallPerDay)) { this.toast.Show(R.strings.enterMaxCallPerDay) }

		else if (isEmpty(this.state.MaxCallPerMin)) { this.toast.Show(R.strings.enterMaxCallPerMin) }

		else if (isEmpty(this.state.MaxCallPerMonth)) { this.toast.Show(R.strings.enterMaxCallPerMonth) }

		else if (isEmpty(this.state.MaxOrderPerSec)) { this.toast.Show(R.strings.enterMaxOrderPerSecond) }

		else if (isEmpty(this.state.MaxRecordReq)) { this.toast.Show(R.strings.enterMaxRecInRequest) }

		else if (isEmpty(this.state.WhiteListIpLimit)) { this.toast.Show(R.strings.enterWhiteListIpAddressLimit) }

		else if (isEmpty(this.state.ConcurrentIpAddressAllow)) { this.toast.Show(R.strings.enterConcurrentIpAddressAllow) }

		else if (isEmpty(this.state.MaxRequestSize)) { this.toast.Show(R.strings.enterMaxRequestSize) }

		else if (isEmpty(this.state.MaxResponseSize)) { this.toast.Show(R.strings.enterMaxResponseSize) }

		else if (isEmpty(this.state.HistoricalData)) { this.toast.Show(R.strings.enterHistoricalData) }
		else {
			...
			<SafeView style={{ flex: 1, backgroundColor: R.colors.background }}>
				...
				<View style={{ flex: 1, justifyContent: 'space-between', }}>
					<ScrollView showsVerticalScrollIndicator={false}>
						<View style={{ paddingLeft: R.dimens.activity_margin, paddingTop: R.dimens.padding_top_bottom_margin, paddingRight: R.dimens.activity_margin, paddingBottom: R.dimens.padding_top_bottom_margin }}>
							...
							{/* Read Only Method List */}
							{
								...
								<View style={{ marginTop: R.dimens.margin, }}>
									<Text style={{ flex: 1, color: R.colors.textPrimary, textAlign: 'center', fontFamily: Fonts.HindmaduraiBold, fontSize: R.dimens.smallText, marginLeft: R.dimens.LineHeight }}>{R.strings.readOnlyApiMethods}</Text>
									{
