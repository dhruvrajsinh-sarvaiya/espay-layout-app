...
class LimitControl extends Component {
    ...
    onSaveButtonPress = async () => {
        ...
        if (!isEmpty(this.state.StartTime) && !isEmpty(this.state.EndTime)) {
            ...
            let trnType = this.state.tabPosition + 1;
            if ((this.state.tabPosition + 1) == 2) { trnType = 9; }
            if ((this.state.tabPosition + 1) == 3) { trnType = 2; }

            //Check NetWork is Available or not
            if (await isInternet()) {
                //call Save Limits Control API
                this.props.onSaveLimits(
                    this.state.AccWalletID,
                    this.state.LimitPerHour,
                    this.state.LimitPerDay,
                    this.state.LimitPerTrn,
                    this.state.LifeTimeLimit,
                    moment(this.state.StartTime, 'hh:mm A').format('X'), moment(this.state.EndTime, 'hh:mm A').format('X'), trnType);
            }
        }
    }
