...
function* getTradeReconListDetail({ payload }) {
    ...
    try {
        //const response = yield call(getTradeReconListRequest, Data);
...
function* getTradePairsDetail({ payload }) {
    ...
    try {
        ...
        //     }
        //     response = yield call(swaggerPostAPI, 'api/TransactionConfiguration/ListPair' + IsMargin, request);
        // } else {
        //     response = yield call(swaggerPostAPI, URL, {});
        // }
		var headers = { 'Authorization': AppConfig.authorizationToken };
        if (request !== undefined) {
            if (request.hasOwnProperty("IsMargin") && request.IsMargin != "" && request.IsMargin != undefined) {
