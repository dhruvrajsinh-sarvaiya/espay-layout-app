...
class NavMenuItem extends Component {
    ...
    render() {
        ...
        if (menu.hasOwnProperty('menu_title')) {
                        ...
                        <Collapse in={menu.open} timeout="auto" className="sub-menu">
                            <List className="list-unstyled py-0">
                                {menu.child_routes.map((subMenu, index) => {
                                    /* permission based menu rendering - added by nishant */
                                    if (subMenu.hasOwnProperty("menu_title")) {
                                            ...
                                            <Fragment key={index}>
                                                ...
                                                <Collapse in={subMenuOpen} timeout="auto">
                                                    {subMenu.child_routes.map((nestedMenu, nestedKey) => {
                                                        /* permission based menu rendering - added by nishant */
                                                        if (nestedMenu.hasOwnProperty("menu_title")) {
                                                            return (<ListItem button component="li" key={nestedKey}>
