...
const columns = [
	{
		name: <IntlMessages id="chathistory.name" />,
		options: { sort: true, filter: false }
	},
	{
		name: <IntlMessages id="chathistory.message" />,
		options: { sort: true, filter: false }
	},
	{
		name: <IntlMessages id="chathistory.time" />,
		options: { sort: true, filter: false }
	}
];
...
class ChatHistory extends Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (typeof nextProps.chatUser_History !== 'undefined' && nextProps.chatUser_History.ReturnCode === 0) {
			if (nextProps.chatUser_History.Data !== null)
				this.setState({ items: this.state.items.concat(nextProps.chatUser_History.Data), currentPage: (this.state.currentPage + 1), totalRecords: nextProps.chatUser_History.TotalRecord, loading: nextProps.loading });
			else
	...
	render() {
		var menudetail = this.checkAndGetMenuAccessDetail('DFEFBDEF-96E9-7971-4D3D-15C12DAC842E');
		if (!menudetail) {
			menudetail = { Utility: [], CrudOption: [] }
		}
		const { loading, err_alert, errors } = this.state;
		return (
			<Fragment>
					...
					<JbsCollapsibleCard fullBlock>
						<div className="StackingHistory">
							...
							<InfiniteScroll
								...
								height={600}
								endMessage={
									this.state.items && this.state.items.length === 0 ? (<div className="row">
							...
							>
								{this.state.items && this.state.items.map((chatdata, index) => {
