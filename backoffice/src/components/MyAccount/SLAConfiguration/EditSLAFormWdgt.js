...
class EditSLAFormWdgt extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: {
  ...
  componentWillMount() {
    var sId = this.props.location.state.id;
    if (sId !== "") {
      this.props.getSLAConfigurationById(sId);
    } else {
      this.props.history.push("/app/my-account/sla-configuration");
    }
  }
  ...
  onSubmit(event) {
    ...
    if (isValid) {
      this.props.editSLAConfiguration(this.state.data);
    }
  }
