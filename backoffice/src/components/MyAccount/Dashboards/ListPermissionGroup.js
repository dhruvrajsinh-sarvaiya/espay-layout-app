...
const columns = [
    ...
    /* {
        name: <IntlMessages id="my_account.roleName" />,
        options: { filter: false, sort: true }
    }, */
    {
...
class ListPermissionGroup extends Component {
    ...
    showComponent = (componentName, permissionGroupID = '', menuDetail) => {
         if (menuDetail.HasChild) {
        //     this.props.getConfigurationRolePermissionGroupById({ ID: permissionGroupID });
            var pageData = {
                PageNo: 0,
                PageSize: AppConfig.totalRecordDisplayInList,
                AllRecords: 1
        ...
        //     this.props.getRolePermissionGroupById({ ID: permissionGroupID });
    ...
    changeStatus() {
        /* var reqObj = {
            PermissionGroupID: this.state.PermissionGroupID,
    ...
    render() {
        const { componentName, open, permissionGrpList, totalCount, pagedata, isModelShow, menuDetail } = this.state;
                        ...
                        <MUIDataTable
                            ...
                            data={permissionGrpList.map((item) => {
                                return [
                                    ...
                                    // changeDateFormat(item.CreatedDate, 'DD-MM-YYYY hh:mm:ss'),
                                    // item.IPAddress,
                                    <PermissionGroupStatus status={item.Status} />,
                                    <Fragment>
                                        {/* {menuPermissionDetail.CrudOption.indexOf('0BB7ACAC') !== -1 && // check edit curd operation
                                        <a href="javascript:void(0)" onClick={(e) => this.showComponent('AddEditPermissionGroup', item.GroupID)} className="text-dark mr-10"><i className="zmdi zmdi-edit zmdi-hc-2x" /></a>}
                                        { (menuPermissionDetail.CrudOption.indexOf('0BB7ACAC') !== -1 && item.Status !== 1) && <a href="javascript:void(0)" onClick={() => this.openModel(item,1)} className="text-dark mr-5"><i className="zmdi zmdi-check zmdi-hc-2x" /></a> }
                                        { (menuPermissionDetail.CrudOption.indexOf('0BB7ACAC') !== -1 && item.Status === 1) && <a href="javascript:void(0)" onClick={() => this.openModel(item,0)} className="text-dark mr-10"><i className="zmdi zmdi-close zmdi-hc-2x" /></a> }
                                        { (menuPermissionDetail.CrudOption.indexOf('B873B896') !== -1 && item.Status !== 9) && <a href="javascript:void(0)" onClick={() => this.openModel(item,9)} className="text-dark mr-10"><i className="zmdi zmdi-delete zmdi-hc-2x" /></a> } */}
                                        {/* {menuPermissionDetail.CrudOption.indexOf('6AF64827') !== -1 && // check view curd operation
                                        <a href="javascript:void(0)" onClick={(e) => this.showComponent('ViewPermissionGroupDetails',item.GroupID)} className="text-dark mr-10"><i className="ti-eye" /></a>} */}
                                        {menuPermissionDetail.CrudOption.indexOf('0BB7ACAC') !== -1 && // check edit curd operation
                                            <a href="javascript:void(0)" onClick={(e) => this.showComponent('AddEditPermissionGroup', item, this.checkAndGetMenuAccessDetail('627E46A0-6181-4C1A-6B15-A1EF259930E8'))} className="text-dark mr-10"><i className="ti-pencil" /></a>}
                                        ...
                                        <a href="javascript:void(0)" onClick={(e) => this.showComponent('SetPermission', item, this.checkAndGetMenuAccessDetail('627E46A0-6181-4C1A-6B15-A1EF259930E8') )} className="text-dark mr-10"><i className="ti-settings" /></a>                                  
                                    </Fragment>
                                ];
                            })}
                        />
