/* 
    ...
    Date : 11-02-2019
    update by  : 
    File Comment : Referral Buy Trade Report Component
*/
...
//Actions
import { buyTradeReport } from 'Actions/MyAccount';
...
class BuyTradeReport extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: {
    ...
    applyFilter = (event) => {
        ...
        if (isValid) {
            this.props.buyTradeReport(newObj);
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.BuyTradeData.ReturnCode === 1) {
            var errMsg = nextProps.BuyTradeData.ErrorCode === 1 ? nextProps.BuyTradeData.ReturnMsg : <IntlMessages id={`apiErrCode.${nextProps.BuyTradeData.ErrorCode}`} />;
            NotificationManager.error(errMsg);
        } else if (nextProps.BuyTradeData.ReturnCode === 0) {
            this.setState({ BuyTradeList: nextProps.BuyTradeData.referral, totalCount: nextProps.BuyTradeData.TotalCount });
            // NotificationManager.success(nextProps.BuyTradeData.ReturnMsg);
        }
    }
    ...
    getListLoginHistory = (PageIndex, Page_Size) => {
        ...
        this.props.buyTradeReport(reqObj);
    }


    render() {
        const { BuyTradeList, errors, totalCount } = this.state;
        ...
        const options = {
            ...
            filter: false,
            download: false,
            serverSide: BuyTradeList.length !== 0 ? true : false,
            page: PageIndex,
            count: totalCount,
        ...
        return (
            <div className="jbs-page-content">
                <DashboardPageTitle title={<IntlMessages id="my_account.ReferralBuyTradeReport" />} breadCrumbData={BreadCrumbData} drawerClose={drawerClose} closeAll={this.closeAll} />
                {loading
                    ?
                    <JbsSectionLoader />
                    :
...
const mapStateToProps = ({ ReferralReport }) => {
    const { BuyTradeData, loading } = ReferralReport;
    return { BuyTradeData, loading };
}
