...
class ListUnassignUserRole extends Component {
    ...
    onChangeSelectUser = (event) => {
        var newObj = Object.assign({}, this.state.data);
        newObj.UserName = (typeof (event.label) === "undefined" ? "" : event.label);
        this.setState({ data: newObj });
    }
    ...
    render() {
        const { componentName, open, UnassignUserList, totalCount, errors, PageData, menuDetail } = this.state;
...
const mapToProps = ({ roleManagementRdcer, drawerclose,authTokenRdcer }) => {
    ...
    return { unassignData, listLoading, drawerclose,        menuLoading,
        menu_rights };
}
