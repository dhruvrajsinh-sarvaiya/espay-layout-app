/*
    ...
    Date : 13-02-2019
    update by  :
    File Comment : Referral Share On Facebook Report Component
*/
...
//Actions
import { shareOnFacebookReport } from 'Actions/MyAccount';
...
class ShareOnFacebookReport extends Component {
    ...
    applyFilter = (event) => {
        ...
        if (isValid) {
            this.props.shareOnFacebookReport(newObj);
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.ShareOnFBData.ReturnCode === 1) {
            var errMsg = nextProps.ShareOnFBData.ErrorCode === 1 ? nextProps.ShareOnFBData.ReturnMsg : <IntlMessages id={`apiErrCode.${nextProps.ShareOnFBData.ErrorCode}`} />;
            NotificationManager.error(errMsg);
        } else if (nextProps.ShareOnFBData.ReturnCode === 0) {
            this.setState({ ShareOnFBList: nextProps.ShareOnFBData.referral, totalCount: nextProps.ShareOnFBData.TotalCount });
            // NotificationManager.success(nextProps.ShareOnFBData.ReturnMsg);
        }
    }
    ...
    getListLoginHistory = (PageIndex, Page_Size) => {
        ...
        this.props.shareOnFacebookReport(reqObj);
    }
    ...
    render() {
        ...
        const { ShareOnFBList, errors, totalCount } = this.state;
        ...
        const options = {
            ...
            filter: false,
            download: false,
            serverSide: ShareOnFBList.length !== 0 ? true : false,
            page: PageIndex,
            count: totalCount,
        ...
        return (
            <div className="jbs-page-content">
                <DashboardPageTitle title={<IntlMessages id="my_account.ShareOnFacebookReport" />} breadCrumbData={BreadCrumbData} drawerClose={drawerClose} closeAll={this.closeAll} />
                {loading
                    ?
                    <JbsSectionLoader />
                    :
...
const mapStateToProps = ({ ReferralReport }) => {
    const { ShareOnFBData, loading } = ReferralReport;
    return { ShareOnFBData, loading };
}
