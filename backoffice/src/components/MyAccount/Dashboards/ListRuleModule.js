...
import { getRuleModuleList, getRuleModuleById, changeStatusRuleModule } from 'Actions/MyAccount';
...
const columns = [
    ...
    {
        name: <IntlMessages id="sidebar.colModuleName" />,
        options: { filter: false, sort: true }
    },
    {
...
class ListRuleModule extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: {
    ...
    showComponent = (componentName, moduleID = '', menuDetail) => {
        ...
        if (moduleID > 0) {
            this.props.getRuleModuleById({ ID: moduleID });
        }
    ...
    getRuleModules = (PageNo, PageSize) => {
        ...
        this.props.getRuleModuleList(reqObj);
    }

    componentWillMount() {
        this.props.getMenuPermissionByID('3F4A1672-1B3A-4FE9-4B20-04786614A5B5'); // get myaccount menu permission
    }

    
    //Pagination Change Method...
    handlePageChange(pageNumber) {
        this.getRuleModules(pageNumber);
    }

    //Row Per Page Change Method...
    onChangeRowsPerPage = event => {
        this.getRuleModules(1, event.target.value);
    };
    ...
    componentWillReceiveProps(nextProps) {
		...
		if (!this.state.menudetail.length && nextProps.menu_rights.hasOwnProperty('ReturnCode')&& this.state.notificationFlag) {
			if (nextProps.menu_rights.ReturnCode === 0) {
                ...
                this.getRuleModules(this.state.data.PageNo, this.state.data.PageSize);
        ...
        if (nextProps.list.hasOwnProperty('Result') && nextProps.list.Result.length > 0) {
            this.setState({ moduleList: nextProps.list.Result, totalCount: nextProps.list.TotalCount });
        }
        ...
        if (nextProps.chngStsData.ReturnCode === 1 || nextProps.chngStsData.ReturnCode === 9) {
            ...
            this.getRuleModules(1, this.state.data.PageSize);
    ...
    changeStatus(Id, Status) {
        ...
        this.props.changeStatusRuleModule(reqObj);
    }
    ...
    render() {
        const { componentName, open, moduleList, totalCount, pagedata } = this.state;
        const { PageNo, PageSize } = this.state.data;
        const { drawerClose } = this.props;

        //Check list permission....
        var menuPermissionDetail = this.checkAndGetMenuAccessDetail('7008d739-51ed-0b8c-540a-98e2c9a063ae'); //7008D739-51ED-0B8C-540A-98E2C9A063AE
        ...
        const options = {
            ...
            filter: false,
            sort: false,
            serverSide: moduleList.length !== 0 ? true : false,
            page: PageNo,
            count: totalCount,
            ...
            downloadOptions: {
                filename: 'RULE_MODULE_LIST_' + changeDateFormat(new Date(), 'YYYY-MM-DD') + '.csv'
            },
            ...
            onTableChange: (action, tableState) => {
                if (action === 'changeRowsPerPage' || action === 'changePage') {
                    this.getRuleModules(tableState.page, tableState.rowsPerPage);
                }
            },
        };
      ...
      {(this.state.menuLoading || this.props.listLoading) && <JbsSectionLoader />}
                <div className="jbs-page-content">
                    <WalletPageTitle title={<IntlMessages id="sidebar.listRuleModule" />} breadCrumbData={BreadCrumbData} drawerClose={drawerClose} closeAll={this.closeAll} />
                    <div className="StackingHistory">
                        <MUIDataTable
                            // title={<IntlMessages id="sidebar.listRuleModule" />} 
                            columns={columns}
                            options={options}
                            data={moduleList.map((item, key) => {
                                return [
                                    ...
                                    <Fragment>
                                        ...
                                        <Fragment>{
                                            this.props.listLoading
                                                ...
                                                : <Switch checked={swtchStatus} onChange={() => this.changeStatus(item.ModuleID, statusValue)} value={swtchStatus} color="primary" />
                                        }</Fragment>
                                        : */}
                                    ...
                                    <div className="list-action">
                                         {menuPermissionDetail.CrudOption.indexOf('0BB7ACAC') !== -1 && //check edit curd operation */}
                                        <a href="javascript:void(0)" onClick={(e) => this.showComponent('AddEditRuleModule', item.ModuleID, this.checkAndGetMenuAccessDetail('7008D739-51ED-0B8C-540A-98E2C9A063AE'))} className="ml-3"><i className="ti-pencil" /></a>
                                         }
                                    </div>
                                ];
                            })}
...
const mapToProps = ({ ruleModuleRdcer, drawerclose,authTokenRdcer }) => {
    //Added by Bharat Jograna (BreadCrumb)13 March 2019
    //To Close the drawer using breadcrumb data 
    ...
    const { list, chngStsData, listLoading } = ruleModuleRdcer;
