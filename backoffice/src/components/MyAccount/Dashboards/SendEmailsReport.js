/*
    ...
    Date : 13-02-2019
    update by  :
    File Comment : Referral Send Emails Report Component
*/
...
//Actions
import { sendEmailReport } from 'Actions/MyAccount';
...
class SendEmailsReport extends Component {
    ...
    applyFilter = (event) => {
        ...
        if (isValid) {
            this.props.sendEmailReport(newObj);
    ...
    clearFilter = () => {
        ...
        this.props.this.props.sendEmailReport(reqObj); (newObj);
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.SendEmailData.ReturnCode === 1) {
            var errMsg = nextProps.SendEmailData.ErrorCode === 1 ? nextProps.SendEmailData.ReturnMsg : <IntlMessages id={`apiErrCode.${nextProps.SendEmailData.ErrorCode}`} />;
            NotificationManager.error(errMsg);
        } else if (nextProps.SendEmailData.ReturnCode === 0) {
            this.setState({ SendEmailList: nextProps.SendEmailData.referral, totalCount: nextProps.SendEmailData.TotalCount });
            // NotificationManager.success(nextProps.SendEmailData.ReturnMsg);
        }
    }
    ...
    getListLoginHistory = (PageIndex, Page_Size) => {
        ...
        this.props.sendEmailReport(reqObj);
    }


    render() {

        const { SendEmailList, errors, totalCount } = this.state;
        ...
        const options = {
            ...
            filter: false,
            download: false,
            serverSide: SendEmailList.length !== 0 ? true : false,
            page: PageIndex,
            count: totalCount,
        ...
        return (
            <div className="jbs-page-content">
                <DashboardPageTitle title={<IntlMessages id="my_account.SendEmailsReport" />} breadCrumbData={BreadCrumbData} drawerClose={drawerClose} closeAll={this.closeAll} />
                {loading
                    ?
                    <JbsSectionLoader />
                    :
...
const mapStateToProps = ({ ReferralReport }) => {
    const { SendEmailData, loading } = ReferralReport;
    return { SendEmailData, loading };
}
