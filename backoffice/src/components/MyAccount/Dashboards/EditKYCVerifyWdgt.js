...
import { editKycVerify } from "Actions/MyAccount";
...
class EditKYCVerifyWdgt extends Component {
	...
	clearData = () => {
		this.setState({ Remark: '', errors: '', menudetail: this.state.menudetail });
	...
	componentWillReceiveProps(nextProps) {
		...
		if (this.state.ntf_flag && (nextProps.data.ReturnCode === 1 || nextProps.data.ReturnCode === 9)) {
			...
			this.setState({ ntf_flag: !this.state.ntf_flag })
		} else if (this.state.ntf_flag && nextProps.data.ReturnCode === 0) {
			if (this.state.listKYC) {
				this.setState({ listKYC: false, ntf_flag: !this.state.ntf_flag });
				this.clearData();
			}
