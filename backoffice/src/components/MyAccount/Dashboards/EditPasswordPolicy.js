...
class EditPasswordPolicyConfig extends Component {
    ...
    resetData = () => {
        ...
        this.setState({ menudetail: this.state.menudetail, handle_flag: this.state.handle_flag })
    }
    ...
    render() {
        ...
        if (this.state.flagPageData) {
            this.setState({ pageValue: pagedata.Id, flagPageData: false });
        }

        if (this.state.pageValue !== 0 && pagedata.Id !== this.state.pageValue) {
            this.setState({ data: pagedata, flag: true, flagPageData: true });
        }

        if (this.state.flag) {
            this.setState({ data: pagedata, flag: false });
        }
