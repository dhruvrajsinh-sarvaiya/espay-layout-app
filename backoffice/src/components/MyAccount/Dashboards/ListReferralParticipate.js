      ...
      {menuPermissionDetail.Utility.indexOf('18736530') !== -1 && //check filter curd operation ? */}
        ...
        <div className="top-filter">
          <Form className="tradefrm row">
            ...
            {/* <FormGroup className="col-md-2 col-sm-4">
              <Button color="primary" disabled={isDisable} onClick={this.getFilterData}><IntlMessages id="widgets.apply" /></Button>
            </FormGroup>
            {this.state.showReset && <FormGroup className="mt-30" style={{ marginRight: "45px" }}>
...
const mapStateToProps = ({ ReferralInvitationsReducer, drawerclose,authTokenRdcer }) => {
  ...
  return { listReferralParticipateData, listServiceData, listChannelTypeData, loading, drawerclose,    menuLoading,
    menu_rights };
}
