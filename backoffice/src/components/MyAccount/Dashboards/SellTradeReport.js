/* 
    ...
    Date : 11-02-2019
    update by  : 
    File Comment : Referral Sell Trade Report Component
*/
...
//Actions
import { sellTradeReport } from 'Actions/MyAccount';
...
const columns = [
    ...
    {
        name: <IntlMessages id="sidebar.coltotalvolume" />,
        options: {
            filter: true,
            sort: true,
        }
    ...
    {
        name: <IntlMessages id="sidebar.colchargecollected" />,
        options: {
            filter: true,
            sort: true,
        }
...
class SellTradeReport extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: {
    ...
    applyFilter = (event) => {
        ...
        if (isValid) {
            this.props.sellTradeReport(newObj);
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.SellTradeData.ReturnCode === 1) {
            var errMsg = nextProps.SellTradeData.ErrorCode === 1 ? nextProps.SellTradeData.ReturnMsg : <IntlMessages id={`apiErrCode.${nextProps.SellTradeData.ErrorCode}`} />;
            NotificationManager.error(errMsg);
        } else if (nextProps.SellTradeData.ReturnCode === 0) {
            this.setState({ SellTradeList: nextProps.SellTradeData.referral, totalCount: nextProps.SellTradeData.TotalCount });
            // NotificationManager.success(nextProps.SellTradeData.ReturnMsg);
        }
    }
    ...
    getListLoginHistory = (PageIndex, Page_Size) => {
        ...
        this.props.sellTradeReport(reqObj);
    }


    render() {
        const { SellTradeList, errors, totalCount } = this.state;
        const { drawerClose, loading } = this.props;
        const { username, Status, FromDate, ToDate, PageIndex, Page_Size } = this.state.data;
        ...
        const options = {
            ...
            filter: false,
            download: false,
            serverSide: SellTradeList.length !== 0 ? true : false,
            page: PageIndex,
            count: totalCount,
        ...
        return (
            <div className="jbs-page-content">
                <DashboardPageTitle title={<IntlMessages id="my_account.ReferralSellTradeReport" />} breadCrumbData={BreadCrumbData} drawerClose={drawerClose} closeAll={this.closeAll} />
                {loading
                    ?
                    <JbsSectionLoader />
                    :
                                ...
                                <div className="top-filter clearfix p-20">
                                    <Form className="tradefrm">
                                        <Row>
                                            ...
                                            <Col sm={2}>
                                                <FormGroup className="mb-5 mt-10">
                                                    <Label for="endDate"><IntlMessages id="sidebar.username" /></Label>
...
const mapStateToProps = ({ ReferralReport }) => {
    const { SellTradeData, loading } = ReferralReport;
    return { SellTradeData, loading };
}
