...
class ListRuleField extends Component {
    ...
    render() {
        const { componentName, open, List, totalCount, pagedata, meu } = this.state;
                    ...
                    <div className="StackingHistory">
                        ...
                        <MUIDataTable
                            ...
                            data={List.map((item, key) => {
                                var status = item.Status === 9 ? 2 : item.Status; //Added by salim dt:27/02/2019
                                return [
                                    ...
                                    <Fragment>
                                        {/* {
                                            menuPermissionDetail.CrudOption.indexOf('0BB7ACAC111') !== -1 //check edit curd operation
                                            ?
                                            <div className="rng_sld slider">
                                                <Slider min={0} max={2} tooltip={false} value={status} onChange={(e) => this.changeStatus(e,item.FieldID)} />
                                            </div>
                                            : */}
...
const mapToProps = ({ ruleFieldRdcer, drawerclose ,authTokenRdcer}) => {
    ...
    return { list, listLoading, chngStsData, drawerclose,        menuLoading,
        menu_rights };
}
