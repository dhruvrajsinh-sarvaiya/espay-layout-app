...
class ListReferralServiceDetail extends Component {
    ...
    getReferralServiceDetail = (PageNo, PageSize) => {
        ...
        // }
        // this.setState({ ...this.state, data: newObj });
    ...
    render() {
        const { componentName, open, ServiceList, totalCount, pagedata, menuDetail, CreditWalletTypeList, CreditWalletTypeLable, SchemeTypeMappingList, SchemeTypeMappingLable } = this.state;
        const { SchemeTypeMappingId, CreditWalletTypeId, Status } = this.state.data;
                                ...
                                return [
                                    ...
                                    <div className="list-action">
                                        {menuPermissionDetail.CrudOption.indexOf('0BB7ACAC') !== -1 && // check for edit permission
                                            <a href="javascript:void(0)" onClick={(e) => this.showComponent('AddEditReferralServiceDetail', item.Id,this.checkAndGetMenuAccessDetail('7D7AA261-93BF-7C10-1A13-5D1F086A12F7'))} className="ml-3"><i className="ti-pencil" /></a>
                                        }
                                        {//menuPermissionDetail.CrudOption.indexOf('0BB7ACAC') !== -1 && // check for edit permission
