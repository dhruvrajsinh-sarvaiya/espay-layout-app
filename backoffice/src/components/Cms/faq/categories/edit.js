...
class EditFaqCategories extends Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (typeof nextProps.localebit != 'undefined' && nextProps.localebit != '' && nextProps.localebit == 1) {
			...
			this.state.faqcategorydetail.locale = locale;
			this.setState({
				faqcategorydetail: this.state.faqcategorydetail
			});
		}
		...
		if (typeof nextProps.faqcategorydata != 'undefined' && typeof nextProps.faqcategorydata.locale != 'undefined' && nextProps.faqcategorydata != '') {
			...
			this.state.faqcategorydetail.locale = newlocale;
			this.state.faqcategorydetail._id = nextProps.faqcategorydata._id;
			this.state.faqcategorydetail.category_id = nextProps.faqcategorydata.category_id;
			this.state.faqcategorydetail.sort_order = nextProps.faqcategorydata.sort_order;
			this.state.faqcategorydetail.status = nextProps.faqcategorydata.status + "";//Change by Khushbu Badheka D:29/01/2019
	...
	render() {
		...
		const { err_alert, err_msg, activeIndex, language, errors, faqcategorydetail, loading, btn_disabled } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
