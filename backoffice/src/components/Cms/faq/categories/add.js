...
class AddFaqCategories extends Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (typeof nextProps.localebit != 'undefined' && nextProps.localebit != '' && nextProps.localebit == 1 && (nextProps.data.responseCode != 9 && nextProps.data.responseCode != 1)) {
			...
			this.state.addNewFaqCategoryDetail.locale = locale;
			this.setState({
				addNewFaqCategoryDetail: this.state.addNewFaqCategoryDetail
			});
		}
	...
	render() {
		...
		const { err_alert, activeIndex, language, errors, addNewFaqCategoryDetail, loading, btn_disabled } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
