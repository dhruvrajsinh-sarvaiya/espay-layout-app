...
class CoinListFields extends Component {
  ...
  toggleCheck = (key, fieldName, value = "") => {
    let tempObj = this.state.coinListField;
    if (fieldName == "status") {
      tempObj[key].status = tempObj[key].status ? 0 : 1;
    } else if (fieldName == "sortOrder") {
      tempObj[key][fieldName] = value;
    }
    this.setState({ coinListField: tempObj });
  }

  componentWillMount() {
    this.props.getMenuPermissionByID('1957EC01-5621-2E40-1124-4952A7E5458B');
  }
  ...
  render() {
    ...
    const { loading, err_alert, errors, coinListField } = this.state;
    const options = {
      responsive: "scroll",
