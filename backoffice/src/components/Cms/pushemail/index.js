...
class PushEmail extends Component {
    ...
    handleChangeMulti = selectedUser => {
        ...
        Address.forEach(function (value) {
            ...
            if (matchBCCuser !== -1 && BCCAddress.length > 0) {
                errorbit = 1;
                NotificationManager.error(<IntlMessages id="pushEmail.error.SameRecipient" />);
                return false;
            } else if (matchCCUser !== -1 && CCAddress.length > 0) {
                errorbit = 1;
                NotificationManager.error(<IntlMessages id="pushEmail.error.SameRecipient" />);
                return false;
            }
    ...
    handleChangeMultiBCC = selectedUser => {
        ...
        BCCAddress.forEach(function (value) {
            ...
            if (matchUser !== -1 && Address.length > 0) {
                errorBit = 1;
    ...
    handleChangeMultiCC = selectedUser => {
        ...
        CCAddress.forEach(function (value) {
            ...
            if (matchUser !== -1 && Address.length > 0) {
                ...
                return false;
            } else if (matchBCCUser !== -1 && BCCAddress.length > 0) {
                errorBit = 1;
                NotificationManager.error(<IntlMessages id="pushEmail.error.SameCCAddress" />);
                return false;
            }
    ...
    onChangeEmailID = (selectedUser) => {
        ...
        if (selectedUser) {
            ...
            //var matchBCCuser = $.inArray(selectedUser, self.state.CCEmailList);
            //var matchCCUser = $.inArray(selectedUser, self.state.BCCEmailList);

            if (matchBCCuser !== -1 && self.state.BCCEmailList.length > 0) {

                self.setState({ EmailList: selectedUser.pop() ? selectedUser : [] });
                NotificationManager.error(<IntlMessages id="pushEmail.error.SameRecipient" />);

            } else if (matchCCUser !== -1 && self.state.CCEmailList.length > 0) {
    ...
    onChangeCCEmailID = (selectedUser) => {
        ...
        if (selectedUser) {
            ...
            //var matchUser = $.inArray(selectedUser, self.state.EmailList);
            //var matchBCCUser = $.inArray(selectedUser, self.state.BCCEmailList);

            if (matchUser !== -1 && self.state.EmailList.length > 0) {

                self.setState({ CCEmailList: selectedUser.pop() ? selectedUser : [] });
                NotificationManager.error(<IntlMessages id="pushEmail.error.SameCCAddress" />);

            } else if (matchBCCUser !== -1 && self.state.BCCEmailList.length > 0) {
    ...
    render() {
        ...
        const { Subject, selectedUser, selectedBCCUser, selectedCCUser, Message, errors, btn_disabled, EmailList, CCEmailList, BCCEmailList } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
...
export default connect(
    mapStateToProps,
    {
        PushEmailRequest, displayUserList, getMenuPermissionByID
    }
