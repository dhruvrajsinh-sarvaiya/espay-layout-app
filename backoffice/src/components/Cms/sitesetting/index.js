...
class SiteSetting extends Component {
	...
	handleselectedFile = event => {
		...
		if (event.target.name == 'logo') {
			...
			// }
			// else if(typeof event.target.files[0]!='undefined' && event.target.files[0].type != 'image/jpeg' && event.target.files[0].type != 'image/jpg' && event.target.files[0].type != 'image/png')
			// {
			// 	errors.logo= 'sitesetting.form.error.validatefile';
			// } 
			// else if(typeof event.target.files[0].size!='undefined' && event.target.files[0].size > 5767168)
			// {
			...
			// 	document.getElementById("logoimgPreview").src= e.target.result;
			// 	//this.setState((state) => ({sitesetting: {...state.sitesetting, image: {...state.sitesetting.image, 'logoPreviewUrl': e.target.result}} }));
			// }
			reader.onloadend = () => {
		...
		if (event.target.name == 'fevicon') {
			...
			let reader = new FileReader();
			// reader.onload = function(e) {
			// 	document.getElementById("feviconimgPreview").src= e.target.result;
			// 	//this.setState((state) => ({sitesetting: {...state.sitesetting, image: {...state.sitesetting.image, 'feviconPreviewUrl': e.target.result}} }))
			// }
			reader.onloadend = () => {
	...
	onChangeAddNewPageDetails(tab, key, value, lang = '', isdefault = 0) {
		if (typeof lang != 'undefined' && lang != '') {
			...
			// if(tab==0)
			// {
			// 	this.setState((state) => ({sitesetting: {...state.sitesetting, general: {...state.sitesetting.general, [key]: value}} }))
			// }
			if (tab == 2) {
	...
	handleSubmit(e) {
		...
		if (!isValid) {
			...
			setTimeout(() => {
				let res = this.props.postSiteSettingInfo(data);
	...
	*/
	...
	render() {
		const { err_alert, activeIndex, languageIndex, countries, stateAll, language, errors, sitesetting, loading, showSuccessStatus, btn_disabled } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
