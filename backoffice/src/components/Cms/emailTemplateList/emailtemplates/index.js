...
const dynamicComponent = (TagName, props, drawerClose, close2Level, closeAll, templatesdata, reload, permission) => {
  return React.createElement(components[TagName], { props, drawerClose, close2Level, closeAll, templatesdata, reload, permission });
};
...
class EmailTemplates extends Component {
  ...
  componentWillMount() {
    this.props.getMenuPermissionByID('E6E1F7B2-3532-1AE7-41D2-006633A1381C');
  }
  ...
  render() {
    var menuPermissionDetail = this.checkAndGetMenuAccessDetail('305DB39B-374F-84DE-6787-729783761922'); //305DB39B-374F-84DE-6787-729783761922
    if (!menuPermissionDetail) {
      menuPermissionDetail = { Utility: [], CrudOption: [] }
    }
    const { loading, err_alert, errors, templateslist, permission } = this.state;
    ...
    const options = {
      ...
      customToolbar: () => {
        if (menuPermissionDetail.CrudOption.indexOf('04F44CE0') !== -1) { // check add curd operation
          return (
            <MatButton
              onClick={(e) => this.showComponent('AddEmailTemplate', (this.checkAndGetMenuAccessDetail('305DB39B-374F-84DE-6787-729783761922')).HasChild)}
    ...
    return (
      <Fragment>
        <div className="jbs-page-content">
                      ...
                      <div className="list-action">
                        {menuPermissionDetail.CrudOption.indexOf('0BB7ACAC') !== -1 &&
                          <a
                            ...
                            color="primary"
                            onClick={(e) => this.showComponent('EditEmailTemplate', (this.checkAndGetMenuAccessDetail('305DB39B-374F-84DE-6787-729783761922')).HasChild, template)}
                          >
          ...
          >
            {/* Added By Megha Kariya (05-02-2019) : Add close2Level */}
            {this.state.componentName != '' && dynamicComponent(this.state.componentName, this.props, this.onClick, this.close2Level, this.closeAll, this.state.templatesdata, this.reload)}
          </Drawer>
        </div>
      </Fragment>
    );
