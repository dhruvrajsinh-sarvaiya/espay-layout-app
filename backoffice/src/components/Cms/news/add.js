...
class AddNews extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(typeof nextProps.localebit != 'undefined' && nextProps.localebit!='' &&  nextProps.localebit==1)
    {
      ...
      this.state.addNewsDetail.locale= locale;
      this.setState({ 
        addNewsDetail:this.state.addNewsDetail
      });
    }
  ...
  render() {
    ...
    const {err_alert, activeIndex,language,errors,addNewsDetail,loading,btn_disabled} = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
