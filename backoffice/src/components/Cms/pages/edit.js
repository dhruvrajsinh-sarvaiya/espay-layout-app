...
class EditPage extends Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (typeof nextProps.localebit != 'undefined' && nextProps.localebit != '' && nextProps.localebit == 1) {
			...
			this.state.pagedetail.locale = locale;
			this.setState({
				pagedetail: this.state.pagedetail
			});
		}

		if (typeof nextProps.pagedata != 'undefined' && typeof nextProps.pagedata.locale != 'undefined' && nextProps.pagedata != '') {
			const newlocale = {};
			{
				nextProps.language && nextProps.language.map((lang, key) => {
			...
			this.state.pagedetail.locale = newlocale;
			this.state.pagedetail._id = nextProps.pagedata._id;
			this.state.pagedetail.layout_id = nextProps.pagedata.layout_id;
			this.state.pagedetail.channel_id = nextProps.pagedata.channel_id;
			this.state.pagedetail.sort_order = nextProps.pagedata.sort_order;
			this.state.pagedetail.page_type = nextProps.pagedata.page_type;
			this.state.pagedetail.route = nextProps.pagedata.route;
			this.state.pagedetail.status = nextProps.pagedata.status + ""; //Change by Khushbu Badheka D:29/01/2019
			this.setState({
				pagedetail: this.state.pagedetail
			});
		}
	...
	render() {
		...
		const { err_alert, err_msg, activeIndex, language, errors, pagedetail, loading, btn_disabled } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
...
const mapStateToProps = ({ languages, staticpages, authTokenRdcer }) => {
	...
	if (typeof staticpages.localebit != 'undefined' && staticpages.localebit != '') {
