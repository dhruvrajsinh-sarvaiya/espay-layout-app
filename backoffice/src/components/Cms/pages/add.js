...
class AddPage extends Component {
	...
	addPageDetail() {
		...
		if (!isValid) {
			...
			setTimeout(() => {
				let res = this.props.addNewPage(data);
	...
	componentWillReceiveProps(nextProps) {
		...
		if (typeof nextProps.localebit != 'undefined' && nextProps.localebit != '' && nextProps.localebit == 1) {
			...
			this.state.addNewPageDetail.locale = locale;
			this.setState({
				addNewPageDetail: this.state.addNewPageDetail
			});
		}
	...
	render() {
		...
		const { err_alert, activeIndex, language, errors, addNewPageDetail, loading, btn_disabled, fieldList } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
