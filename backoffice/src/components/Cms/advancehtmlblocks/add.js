 ...
 * Componeent For Add Advance HTML Blocks
 */
...
import { getAdvanceHTMLBlocksList, addAdvanceHTMLBlock } from 'Actions/AdvanceHTMLBlocks';//call Actions for Add and list 
import { DashboardPageTitle } from '../DashboardPageTitle';//use for UI perpose in Page Header
...
const BreadCrumbData = [
    ...
    {
        title: <IntlMessages id="advanceHtmlBlock.title.add-AdvanceHtmlBlock" />,
        link: '',
        index: 0
    }
];
...
class add extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.add_advance_htmlblock.responseCode === 0) {
            NotificationManager.success(<IntlMessages id="advanceHtmlBlock.addHTMLBlockSuccess" />);
            this.props.getAdvanceHTMLBlocksList();
            this.resetData();
        } else if (nextProps.add_advance_htmlblock.responseCode === 1 || nextProps.add_advance_htmlblock.responseCode === 9) {
    ...
    onAddBlock = (event) => {
        ...
        if (isValid || Object.keys(errors).length === 0) {            
            ...
            this.props.addAdvanceHTMLBlock(Obj);
        }
    }
    ...
    render() {
        ...
        return (
            <div className="jbs-page-content">
                <DashboardPageTitle title={<IntlMessages id="advanceHtmlBlock.title.add-AdvanceHtmlBlock" />} breadCrumbData={BreadCrumbData} drawerClose={this.resetData} closeAll={this.closeAll} />
...
const mapDispatchToProps = ({ AdvanceHTMLBlocksReducer }) => {
    const { add_advance_htmlblock, loading } = AdvanceHTMLBlocksReducer;
    return { add_advance_htmlblock, loading };
};
