...
class EditHelpManual extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (typeof nextProps.localebit != 'undefined' && nextProps.localebit != '' && nextProps.localebit == 1) {
      ...
      this.state.helpmanualdetail.locale = locale;
      this.setState({
        helpmanualdetail: this.state.helpmanualdetail
      });
    }
    ...
    if (typeof nextProps.manualdata != 'undefined' && typeof nextProps.manualdata.locale != 'undefined' && nextProps.manualdata != '') {
      ...
      this.state.helpmanualdetail.locale = newlocale;
      this.state.helpmanualdetail._id = nextProps.manualdata._id;
      this.state.helpmanualdetail.manual_id = nextProps.manualdata.manual_id;
      this.state.helpmanualdetail.sort_order = nextProps.manualdata.sort_order;
      this.state.helpmanualdetail.status = nextProps.manualdata.status + "";  //Change by Khushbu Badheka D:30/01/2019
      this.state.helpmanualdetail.module_id = nextProps.manualdata.module_id._id ? nextProps.manualdata.module_id._id : this.state.helpmanualdetail.module_id;
  ...
  render() {
    ...
    const { err_alert, err_msg, activeIndex, language, errors, helpmanualdetail, loading, modulelist, btn_disabled } = this.state; // Added By Megha Kariya (08/02/2019) : add btn_disabled
