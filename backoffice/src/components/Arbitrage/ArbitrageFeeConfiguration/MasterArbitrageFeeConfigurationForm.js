 ...
 *   Date: 10-06-2019
 *   Component:  Fee Configuration Form
 */
...
import MasterFormValidation from "Validations/Arbitrage/ArbitrageFeeConfiguration/MasterFormValidation";
import { NotificationManager } from "react-notifications";
import IntlMessages from "Util/IntlMessages";
import { ListFeeConfiguration,addFeeConfiguration,UpdateFeeConfiguration} from 'Actions/Arbitrage/ArbitrageFeeConfiguration';
import {
...
class MasterArbitrageFeeConfigurationForm extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (
            nextProps.getFeeById.ReturnCode === 0 &&
            nextProps.Form
        ) {
            this.setState({
                addRecord: {
                    Id: nextProps.getFeeById.Details.Id,
                    WalletTypeId: nextProps.getFeeById.Details.WalletTypeID,
                    TrnType: nextProps.getFeeById.Details.TrnType,
                    KYCComplaint: nextProps.getFeeById.Details.KYCComplaint,
                    Remarks: nextProps.getFeeById.Details.Remarks,
                    Status: nextProps.getFeeById.Details.Status,
                    SpecialChargeConfigurationID: nextProps.getFeeById.Details.SpecialChargeConfigurationID,
                    SlabType: nextProps.getFeeById.Details.SlabType,
                    Pair: nextProps.getFeeById.Details.Pair,
                    SerProId: nextProps.getFeeById.Details.SerProId,
                },
            });
        }
        // validate success foe addRecord
        if (nextProps.addFeeData.hasOwnProperty("ReturnCode") && this.state.notificationFlag) {
            this.setState({ notificationFlag: false });
            if (nextProps.addFeeData.ReturnCode === 0) {
                //success
                ...
                setTimeout(function () {
                    ...
                    //list action call
                    if (nextProps.FeeConfigurationData.length > 0) {
                        this.props.ListFeeConfiguration({});
                    }
                }.bind(this), 1000);
            } else if (nextProps.addFeeData.ReturnCode !== 0) {
                NotificationManager.error(intl.formatMessage({ id: `apiWalletErrCode.${nextProps.addFeeData.ErrorCode}` }));
            }
        }
    ...
    onSubmitaddFeeConfigForm() {
        const { errors, isValid } = MasterFormValidation(this.state);
        this.setState({ errors: errors, notificationFlag: true });
        if (isValid) {
            const { addRecord } = this.state;
            delete addRecord.Id;
            this.props.addFeeConfiguration(addRecord);
        }
    }
    ...
    onUpdateFeeConfiguration() {
        ...
        if (isValid) {
            ...
            this.props.UpdateFeeConfiguration(addRecord);
        ...
        return (
            <div className="jbs-page-content">
                <form>
                    ...
                    {((Form ? (menuDetail["3C0B3758-04AE-5C6E-484C-F7C4F17D42BF"] && menuDetail["3C0B3758-04AE-5C6E-484C-F7C4F17D42BF"].Visibility === "E925F86B") : (menuDetail["82B59FFB-29E6-09A3-9B4B-9717E4195801"] && menuDetail["82B59FFB-29E6-09A3-9B4B-9717E4195801"].Visibility === "E925F86B")) && this.state.pairflag) &&
                        ...
                        <div className="form-group row">
                            <Label className="col-md-4 col-form-label  d-inline">
                                {intl.formatMessage({
                                    id: "sidebar.tradingLedger.filterLabel.currencyPair"
                            ...
                            <div className="col-md-8">
                                <Input
                                    ...
                                    onChange={e =>
                                        this.onChangeAddText("Pair", e.target.value)
                                    }
                                >
                                    <option value="">
                                        {intl.formatMessage({
                                    ...
                                    {this.props.serviceProvider.length && this.props.serviceProvider.map((item, key) => (
                                            <option value={item.Id} key={key}>{item.ProviderName}</option>
                                        ))}
                                </Input>
                                {errors.Pair && (
                                    <span className="text-danger">
                                        {intl.formatMessage({ id: errors.Pair })}
                                    </span>
                                )}
                            </div>
                        </div>
                    ...
                    {(Form ? (menuDetail["3C0B3758-04AE-5C6E-484C-F7C4F17D42BF"] && menuDetail["3C0B3758-04AE-5C6E-484C-F7C4F17D42BF"].Visibility === "E925F86B") : (menuDetail["82B59FFB-29E6-09A3-9B4B-9717E4195801"] && menuDetail["82B59FFB-29E6-09A3-9B4B-9717E4195801"].Visibility === "E925F86B")) &&
                     <div className="form-group row">
                                    <Label className="col-md-4 col-form-label  d-inline"><IntlMessages id="liquidityprovider.list.option.label.serviceprovider" /><span className="text-danger"> *</span></Label>
                                    <div className="col-md-8">
                                    <Input className="form-control" type="select" name="SerProId" id="SerProId" value={addRecord.SerProId} onChange={e => this.onChangeAddText("SerProId", e.target.value)}>
                                    ...
                                    {errors.SerProId && (
                                    <span className="text-danger">
                                        {intl.formatMessage({ id: errors.SerProId })}
                                    </span>
                                )}
                                </div>
                            </div>
                    }
                    {((Form ? (menuDetail["73B8913F-42FC-48C7-28AE-504664AE774B"] && menuDetail["73B8913F-42FC-48C7-28AE-504664AE774B"].Visibility === "E925F86B") : (menuDetail["B0D7DCDC-2F03-8408-969E-BE50CDA7263B"] && menuDetail["B0D7DCDC-2F03-8408-969E-BE50CDA7263B"].Visibility === "E925F86B")) && this.state.currencyFlag) &&
                        ...
                        <div className="offset-md-4 col-md-8 offset-sm-3 col-sm-9 col-xs-12">
                            <div className="btn_area">
                                <Button
                                    ...
                                    onClick={() => this.onUpdateFeeConfiguration()}
...
MasterArbitrageFeeConfigurationForm.defaultProps = {
    Form: false
};

//map method
const mapStateToProps = ({ FeeConfigurationReducer,tradeRecon, walletUsagePolicy, transactionPolicy, authTokenRdcer,liquidityManager, ArbitrageCurrencyConfiguration }) => {
    const {
...
)(injectIntl(MasterArbitrageFeeConfigurationForm));
