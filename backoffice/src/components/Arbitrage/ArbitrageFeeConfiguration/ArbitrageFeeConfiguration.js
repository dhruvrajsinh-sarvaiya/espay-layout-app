...
import ArbitrageFeeConfigurationForm from "./ArbitrageFeeConfigurationForm";
...
import { getFeeConfigurationList,updateFeeConfigurationList,getFeeConfigurationDetailById} from 'Actions/Arbitrage/ArbitrageFeeConfiguration';
//Action methods..
import {
...
//components name
const components = {
    ArbitrageFeeConfigurationForm: ArbitrageFeeConfigurationForm,
};
...
const BreadCrumbData = [
    ...
    {
        title: <IntlMessages id="lable.ArbitrageFeeConfigurationDetails" />,
        link: '',
        index: 2
    },
];

class ArbitrageFeeConfiguration extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.updateDetails.hasOwnProperty("ReturnCode") && this.state.notificationFlag) {
            ...
            if (nextProps.updateDetails.ReturnCode === 0) {
                ...
                setTimeout(function () {
                    this.props.getFeeConfigurationList({ MasterId: this.state.MasterId })
                }.bind(this), 3000);
    ...
    onEditChargeConfig(item, menuDetail) {
        ...
        if (menuDetail) {
            this.setState({
                componentName: "ArbitrageFeeConfigurationForm",
                open: true,
                Form: true,
                // menuDetail:menuDetail
            });
            this.props.getFeeConfigurationDetailById(
    ...
    onDelete = () => {
        ...
        if (deleteRecord) {
            ...
            this.props.updateFeeConfigurationList(deleteRecord);
        }
    }
    ...
    render() {
                ...
                if (menuPermissionDetail.CrudOption.indexOf('04F44CE0') !== -1) { // check add curd operation
                    return (
                        <Button
                            ...
                            // style={{ float: "right" }}
                            onClick={() => this.showComponent("ArbitrageFeeConfigurationForm", (this.checkAndGetMenuAccessDetail('FAA37144-6DFF-7514-4273-A6C349C270DE').HasChild)  //24DC250B-A773-3FF0-5B86-8381F6906D59
                            )}
                        >
        ...
        return (
            <div className="jbs-page-content">
                ...
                <WalletPageTitle title={intl.formatMessage({ id: "lable.ArbitrageFeeConfigurationDetails" })} breadCrumbData={BreadCrumbData} drawerClose={drawerClose} closeAll={this.closeAll} />
...
//map method
const mapStateToProps = ({ FeeConfigurationReducer, authTokenRdcer }) => {
    const {
        menuLoading,
        menu_rights
    } = authTokenRdcer;
    const { Details, loading, addDetails, getDetails, updateDetails } = FeeConfigurationReducer;
    return {
...
export default connect(mapStateToProps,{
      ...
      getFeeConfigurationDetailById, 
      updateFeeConfigurationList, 
      getMenuPermissionByID 
    }
)(injectIntl(ArbitrageFeeConfiguration));
