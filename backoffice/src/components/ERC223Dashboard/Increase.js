...
class Increase extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.Increase.hasOwnProperty("ReturnCode") && this.state.notificationFlag) {
            if (nextProps.Increase.ReturnCode !== 0) {
                NotificationManager.error(<IntlMessages id={`apiWalletErrCode.${nextProps.Increase.ErrorCode}`} />);
            }
            else if (nextProps.Increase.ReturnCode === 0) {
...
const mapStateToProps = ({ IncreaseReducer, walletUsagePolicy, drawerclose, authTokenRdcer, dipositReportReducer }) => {
    ...
    const { Increase,IncreaseList, loading } = IncreaseReducer;
    const { walletType } = walletUsagePolicy;
    const {
        menuLoading,
        menu_rights
    } = authTokenRdcer;
    return { loading, walletType, drawerclose, menuLoading, menu_rights, Increase, reconResponse, response2fa, errors, error2fa,IncreaseList };
};
