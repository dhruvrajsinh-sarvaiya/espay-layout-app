...
class CoinConfiguration extends Component {
    ...
    componentWillReceiveProps(nextprops) {
        // added by parth andhariya
        // if (nextprops.CurrencyLogo && nextprops.CurrencyLogo.ReturnCode === 0 && this.state.notificationFlag) {
        //     NotificationManager.success(nextprops.CurrencyLogo.ReturnMsg);
        //     this.setState({ notificationFlag: false })
        // }
        if (nextprops.coinConfigurationList) {
            this.setState({
                coinConfigurationList: nextprops.coinConfigurationList
            })
        }
    ...
    // invoke after render method call
    // componentDidMount() {
    //     //added by parth andhariya
    //     if (this.state.ConfigurationShowCard === 1) {
    //         this.props.getCoinConfigurationList({ IsMargin: 1 });
    ...
    render() {
        ...
        const columns = [
            ...
            {
                name:
                    <IntlMessages id="manageMarkets.list.form.label.code" />
                ,
                options: { sort: true, filter: true }
            },
            {
            ...
            {
                name:
                    <IntlMessages id="coincofiguration.issuedate" />
                ,
                options: { sort: true, filter: true }
            },
            {
                        ...
                        data={this.state.coinConfigurationList.length &&
                            this.state.coinConfigurationList.map((coinDetail, key) => {
                                ...
                                return [
                                    ...
                                    <img
                                        // src={AppConfig.coinlistImageurl + '/' + coinDetail.SMSCode + '.png'}
