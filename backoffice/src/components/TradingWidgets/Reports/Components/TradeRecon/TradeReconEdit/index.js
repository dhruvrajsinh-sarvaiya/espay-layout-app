  ...
  onSearchList = (event, value) => {
    ...
    if (searchText !== "") {
      if (this.state.activeOrders.length) {
        this.state.activeOrders.map(value => {
          if (value.Price.toString().indexOf(searchText) !== -1) {
    ...
    return (
      <Fragment>
        <div className="mb-10  charts-widgets-wrapper">
          ...
          {/* <PageTitleBar
            title={<IntlMessages id="traderecon.newpage.label.title" />}
          ...
          {info !== null && info !== undefined ? (
            <MUIDataTable
              ...
              // data={[info.PairName, info.Price, info.Amount]}
              columns={columns}
              options={options}
            />
          ) : (
