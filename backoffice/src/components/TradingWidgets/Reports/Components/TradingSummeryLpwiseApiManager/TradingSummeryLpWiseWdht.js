...
class TradingSummaryLpWise extends Component {
  constructor(props) {
    super(props);
    this.state = {
      start_date: this.props.state
        ? this.props.state.FromDate
        : new Date().toISOString().slice(0, 10),
      end_date: this.props.state
        ? this.props.state.ToDate
        : new Date().toISOString().slice(0, 10),
      ...
      reportTitle: this.props.state
        ? this.props.state.reportTitle
        : "sidebar.tradeRouting",
      loading: false,
      pairList: [],
      tradingLedger: [],
      isChange: false,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.displayCustomerData &&
      nextProps.error.length == 0 &&
      this.state.tradeLedgerlpBit !== nextProps.tradeLedgerlpBit
    ) {
      this.setState({
      ...
      NotificationManager.error(
        <IntlMessages
          id={`error.trading.transaction.${nextProps.error.ErrorCode}`}
        />
      );
      this.setState({
        tradingLedger: [],
...
class TradeSummaryLPwiseCollapse extends Component {
  constructor(props) {
    super(props);
    this.state = {
      collapse: false
  ...
  render() {
    ...
    const { collapse } = this.state;
    {
      var price =
        data.OrderType == "MARKET" ? (
          <IntlMessages id="trading.admin.markets.tab.market" />
        ) : (
            data.Price
          );
    }
    return (
      <Fragment>
        {/* {this.props.userListLoading && <JbsSectionLoader />} */}
        <tr
        ...
        >
          ...
          <td>{data.Price == 0 ? price : parseFloat(data.Price).toFixed(8)}</td>
                ...
                <Table hover className="mb-0 tradetable">
                  <thead>
                    <tr>
                      ...
                      <th width="25%">
                        <IntlMessages
                          id={"sidebar.Tradingsummarylpwise.tableHeading.price"}
                        />
                      </th>
                      <th width="25%">
                        <IntlMessages
...
export default connect(
  mapStateToProps,
  { getTradingSummeryLpwiseList, getTradePairs, getUserDataList, getMenuPermissionByID }
