...
module.exports = function validateRouteRequest(data, TrnType) {
    ...
    let routeFlag = false;
    let routesError = [];
    let ConvertAmount = data.ConvertAmount + "";
    let ConfirmationCount = data.ConfirmationCount + "";
    ...
    // }
    /* // asset name
    if (validator.isEmpty(data.AssetName)) {
        errors.AssetName = "wallet.errAssetName";
    }
    // wallet id 
    if (validator.isEmpty(data.ProviderWalletID)) {
        errors.ProviderWalletID = "wallet.errWalletId";
    }
    // convert amount
    if (validator.isEmpty(ConvertAmount)) {
        errors.ConvertAmount = "wallet.errConvetAmt";
    }
    // confirmation count
    if (validator.isEmpty(ConfirmationCount)) {
        errors.ConfirmationCount = "wallet.errConfirmationCnt";
    } */
    // route selection
