...
module.exports = function validateHelpManualInput(data) {
  ...
  Object.keys(data.locale).forEach((lg, index) => {
    ...
    else if( typeof (data.locale[lg].title) !== 'undefined' && (data.locale[lg].title) !== "" && data.locale[lg].title.length > 150) { // Added By Megha Kariya (15/02/2019)
    ...
    if (!validator.isEmpty(data.locale[lg].content)){
      ...
      else if (isHtmlTag(data.locale[lg].content)) {
         errors[lg].content= "my_account.err.htmlTag";
        localeerror=true;
      }
    }
    if(!localeerror)
  ...
  // else if(typeof(data.module_id) != 'undefined' && !validator.isNumeric(data.module_id) || parseInt(data.module_id) == 0) {
  //   errors.module = "helpmanualform.error.validhelpmodule";
  // }
